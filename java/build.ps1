...
if (Test-Path -LiteralPath $javaExe) {
    ...
    $vendor = ([regex]::Match($props, '(?m)^\s*java\.vendor = (.+)$')).Groups[1].Value.Trim()
    $runtime = ([regex]::Match($props, '(?m)^\s*java\.runtime\.version = (.+)$')).Groups[1].Value.Trim()
    $jdk = "$vendor $runtime".Trim()
}
