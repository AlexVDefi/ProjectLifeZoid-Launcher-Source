...
public class IsoWindow extends IsoObject implements BarricadeAble, Thumpable {
    ...
    public static boolean canClimbThroughHelper(IsoGameCharacter chr, IsoGridSquare sq, IsoGridSquare oppositeSq, boolean north) {
        ...
        if (testSquare == null) {
            ...
            return false;
        // PLZ PATCH: a sealed home refuses a climb the way it refuses a step. Every route into a
        // building that is not a doorway funnels through this method - IsoWindow, IsoThumpable and
        // IsoWindowFrame all call it from canClimbThrough, climbOverFence calls it directly, and
        // ClimbSheetRopeState calls it for the window or fence at the top of the rope - and none of
        // them ever reaches testCollideAdjacent, because a climb WRITES the character's position
        // instead of stepping into the square. Without this line an open or broken window is a hole
        // in the seal, which is exactly what a burglar looks for.
        //
        // It sits one branch above vanilla's own safehouse refusal below, and deliberately outside
        // it: that one is skipped entirely on the "nothing to stand on" branch, and this refusal
        // must not be conditional on the far side being comfortable. testSquare is the DESTINATION,
        // so climbing OUT of a sealed house is untouched.
        } else if (PLZGatedGrid.refusesEntry(testSquare, chr)) {
            return false;
        // END PLZ PATCH
        } else {
