...
public class IsoDoor extends IsoObject implements BarricadeAble, Thumpable, IHasHealth, ILockableDoor, ICurtain {
    ...
    public boolean couldBeOpen(IsoGameCharacter chr) {
        if (chr instanceof IsoAnimal) {
            ...
            return false;
        } else if (this.plzAbsolutelyLocked()) {
            return false;
    ...
    public void ToggleDoorActual(IsoGameCharacter chr) {
        ...
        if (chr != null && !(chr instanceof IsoAnimal)) {
            ...
            boolean plzLocked = this.plzAbsolutelyLocked();

            if (!plzLocked && GameServer.server && chr instanceof IsoPlayer && player.getRole().hasCapability(Capability.CanOpenLockedDoors)) {
                this.locked = false;
                this.setLockedByKey(false);
            }

            if (!plzLocked && Core.debug && DebugOptions.instance.cheat.door.unlock.getValue()) {
                this.locked = false;
                this.setLockedByKey(false);
            }
            ...
            if (this.isBarricaded()) {
                ...
                if (!plzLocked && this.canBeOpenFromInside(chr)) {
                    this.locked = false;
                    this.setLockedByKey(false);
                }

                if (chr instanceof IsoPlayer
                    && (plzLocked || this.isLockedByKey()
                        || this.getModData().rawget("CustomLock") instanceof Boolean customLock && customLock)
                    && !this.isOpen()) {
                    if (plzLocked || !this.plzMayOpen(chr)) {
                ...
                boolean bUnlock = this.plzMayOpen(chr);
    ...
    private boolean plzSenderCannotUseDoors() {
        ...
        IsoPlayer sender = GameServer.IDToPlayerMap.get(this.lastPlayerOnlineId);
        if (sender == null) {
            return false;
        }

        return PLZGatedGrid.hasFlag(sender.getUsername(), PLZGatedGrid.NO_DOORS_FLAG);
    }

    private boolean plzSenderMayUnlock() {
        if (this.plzAbsolutelyLocked()) {
            return false;
        }
        ...
        if (sender.getRole() != null && sender.getRole().hasCapability(Capability.CanOpenLockedDoors)) {
            return true;
        }

        if (this.canBeOpenFromInside(sender)) {
            return true;
        }

        return this.plzMayOpen(sender);
    }
    ...
    private boolean plzMayOpen(IsoGameCharacter chr) {
        if (chr == null) {
            return false;
        }

        IsoPlayer player = Type.tryCastTo(chr, IsoPlayer.class);
        if (player != null && PLZGatedGrid.hasFlag(player.getUsername(), PLZGatedGrid.NO_DOORS_FLAG)) {
            return false;
        }

        if (chr.getInventory() != null && chr.getInventory().haveThisKeyId(this.getKeyId()) != null) {
            return true;
        }

        return PLZDoorAccess.mayOpen(player, this.getKeyId());
    }
    ...
    private boolean plzRefuseUnlock(boolean bOpen, boolean bLocked, boolean bLockedByKey) {
        ...
        boolean keyLocked = this.isLockedByKey()
            || this.hasModData() && this.getModData().rawget("CustomLock") instanceof Boolean customLock && customLock;
        if (!keyLocked || this.isHoppable()) {
            return false;
        }
        ...
        return !this.plzSenderMayUnlock();
    }
