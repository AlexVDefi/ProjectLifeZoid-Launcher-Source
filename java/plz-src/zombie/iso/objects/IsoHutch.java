...
public class IsoHutch extends IsoObject {
               ...
               if (!checkedSquares.contains(sq2)) {
                  ...
                  for (int i = 0; i < sq2.getSpecialObjects().size(); i++) {
                     if (sq2.getSpecialObjects().get(i) instanceof IsoHutch hutch) {
                        INetworkPacket.sendToRelative(PacketType.AddItemToMap, this.square.x, this.square.y, hutch);
                     }
                  }
               }
            }
   ...
   public void update() {
      if (!this.isSlave() && this.isExistInTheWorld()) {
         if (this.isOwner()) {
            ...
            if (!this.animalInside.isEmpty() && this.plzRollFloorDirt(prob)) {
               ...
               if (animal != null) {
                  if (animal.nestBox > -1) {
                     ...
                     if (animal.getData().getHutchPosition() != hutchPosition) {
                        DebugType.Animal.warn("animal hutchPosition %d != animalInside index %d".formatted(animal.getData().getHutchPosition(), hutchPosition));
   ...
   public void doMeta(int hours) {
      for (int i = 0; i < hours; i++) {
         int prob = 25 - (this.animalInside.size() + this.plzOutsideResidents());
         if (prob > 10) {
            prob = 10;
         }
   ...
   private void updateAnimalHealthInside(IsoAnimal animal, boolean hourGrow) {
      ...
      if (dirt > 20.0F && Rand.NextBool(250 - (int)dirt)) {
         animal.setHealth(animal.getHealth() - 0.01F * (dirt / 1000.0F) * this.plzDrainMultiplier());
      }
   ...
   // PLZ: vanilla's rates are per tick and per real second, tuned at 60 fps on a 90-minute day.
   private static final float PLZ_BASELINE_GAME_SECONDS_PER_TICK = 86400.0F / (90.0F * 60.0F * 60.0F);

   private static float plzGameTimeScale() {
      return GameTime.getInstance().getGameWorldSecondsSinceLastUpdate() / PLZ_BASELINE_GAME_SECONDS_PER_TICK;
   }

   private boolean plzRollFloorDirt(int prob) {
      if (!PLZFixes.on(PLZFixes.HUTCH_GAME_TIME)) {
         return Rand.NextBool(prob);
      }

      PLZFixes.hit(PLZFixes.HUTCH_GAME_TIME);
      return Rand.NextBool(plzGameTimeScale() / prob);
   }

   private float plzDrainMultiplier() {
      if (!PLZFixes.on(PLZFixes.HUTCH_GAME_TIME)) {
         return GameTime.getInstance().getMultiplier();
      }

      PLZFixes.hit(PLZFixes.HUTCH_GAME_TIME);
      return 0.8F * plzGameTimeScale();
   }

   // PLZ: animalOutside is every animal in the zone, so a pen of cows filled a chicken hutch at 100% per hour.
   private int plzOutsideResidents() {
      if (!PLZFixes.on(PLZFixes.HUTCH_META_RESIDENTS)) {
         return this.animalOutside.size();
      }

      PLZFixes.hit(PLZFixes.HUTCH_META_RESIDENTS);
      int residents = 0;

      for (int i = 0; i < this.animalOutside.size(); i++) {
         IsoAnimal animal = this.animalOutside.get(i);
         if (animal != null && animal.adef != null && animal.adef.hutches != null && animal.adef.hutches.contains(this.type)) {
            residents++;
         }
      }
