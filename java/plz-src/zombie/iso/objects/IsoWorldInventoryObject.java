...
public class IsoWorldInventoryObject extends IsoObject implements IItemProvider {
    ...
    /** PLZ: rebuild a null sprite in place so render() has something to draw. See render(). */
    private void plzRestoreSprite() {
        try {
            this.sprite = IsoSprite.CreateSprite(IsoSpriteManager.instance);
            if (this.getItem() != null) {
                this.updateSprite();
            ...
            zombie.plz.PLZFixes.hit(zombie.plz.PLZFixes.WORLD_ITEM_SPRITE_GUARD);
        } catch (Throwable failure) {
            zombie.debug.DebugLog.log("PLZFixes: null-sprite repair failed, vanilla render resumes: " + failure);
        }
    }

    @Override
    public void render(float x, float y, float z, ColorInfo col, boolean bDoChild, boolean bWallLightingPass, Shader shader) {
        if (this.sprite == null && zombie.plz.PLZFixes.on(zombie.plz.PLZFixes.WORLD_ITEM_SPRITE_GUARD)) {
            // PLZ: receiving an UpdateItemSprite for a runtime-created sprite leaves this object's
            // sprite permanently null on the client, and the unguarded deref below then throws on
            // EVERY frame, killing the whole FBORenderCell tile pass for the rest of the session.
            // Only a reconnect rebuilds the object. Rebuild a usable sprite instead.
            //
            // This is the receiving half of spriteTransmitGuard: that one stops the server sending
            // an unresolvable sprite, this one repairs a client that already has one - including
            // clients talking to an unpatched server.
            plzRestoreSprite();
        }
