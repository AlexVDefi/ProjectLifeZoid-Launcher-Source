...
public class IsoThumpable extends IsoObject implements BarricadeAble, Thumpable, IHasHealth, ILockableDoor {
    ...
    /** getObjectIndex() is square.getObjects().indexOf(this), a linear scan run per object per tick. */
    private boolean plzUpdateIsIdle() {
        if (this.getLifeLeft() > -1.0F) {
            return false;
        }
        if (GameServer.server) {
        ...
        return this.lightSourceFuel == null || this.lightSourceFuel.isEmpty();
    }

    @Override
    public void update() {
        if (PLZObjectUpdate.skipIdle && this.plzUpdateIsIdle()) {
            PLZObjectUpdate.countSkipped();
            return;
        }

        PLZObjectUpdate.countRan();
        if (this.getObjectIndex() != -1) {
            if (!GameServer.server) {
