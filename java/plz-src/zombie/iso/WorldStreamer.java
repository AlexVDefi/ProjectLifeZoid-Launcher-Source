...
import zombie.plz.PLZStreamerWake;
...
public final class WorldStreamer {
    ...
    public void create() {
        if (this.worldStreamer == null && !GameServer.server) {
            this.finished = false;
            this.worldStreamer = new Thread(ThreadGroups.Workers, () -> {
                PLZStreamerWake.register();
    ...
    public boolean isBusy() {
        return !GameClient.client
            ...
            ? !this.jobQueue.isEmpty() || !this.jobList.isEmpty() || PLZRecalcPool.inFlight() > 0
            : true;
    }
                    ...
                    if (this.bb1.limit() == this.bb2.limit()) {
                        ...
                        for (int r = 0; r < sq1.getErosionData().regions.size(); r++) {
                            if (sq1.getErosionData().regions.get(r).dispSeason != sq2.getErosionData().regions.get(r).dispSeason) {
                                DebugLog.log("season1=" + sq1.getErosionData().regions.get(r).dispSeason + " season2=" + sq2.getErosionData().regions.get(r).dispSeason);
                                seasonMatch = false;
                            }
                        }
                    }
