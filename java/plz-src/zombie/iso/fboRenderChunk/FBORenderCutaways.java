...
public final class FBORenderCutaways {
    private final IdentityHashMap<IsoGridSquare, Integer> plzOldFlags = new IdentityHashMap<>();
    private final ArrayList<IsoGridSquare> plzTouched = new ArrayList<>();
    ...
    public static final ObjectPool<FBORenderCutaways.CutawayWall> s_cutawayWallPool = new ObjectPool<>(
        FBORenderCutaways.CutawayWall::new, "FBORenderCutaways.s_cutawayWallPool", 8192
    );
    public static final ObjectPool<FBORenderCutaways.SlopedSurface> s_slopedSurfacePool = new ObjectPool<>(
        FBORenderCutaways.SlopedSurface::new, "FBORenderCutaways.s_slopedSurfacePool", 1024
    );
    public static long plzRoofFlipsHeld;
    public static long plzCutawayVisits;
    public static long plzCutawayChunksInvalidated;
    public static long plzCutawayChangedSquares;
    public static long plzWallsSkipped;
    public static long plzWallsVisited;
    ...
    boolean checkOrphanStructures(int playerIndex, IsoChunk chunk) {
        ...
        for (int z2 = PZMath.max(1, chunk.minLevel); z2 <= chunk.maxLevel; z2++) {
            ...
            if (orphanStructures.hasOrphanStructures) {
                boolean plzWantHide = orphanStructures.shouldCutaway();
                int plzDebounce = PLZPerf.ROOF_HIDE_DEBOUNCE_FRAMES;
                if (plzDebounce > 0 && !orphanStructures.isPlayerInRange(playerIndex, FBORenderCutaways.PlayerInRange.Unset)) {
                    boolean plzIsHidden = orphanStructures.isPlayerInRange(playerIndex, FBORenderCutaways.PlayerInRange.True);
                    if (plzWantHide == plzIsHidden) {
                        orphanStructures.plzPendingFrames[playerIndex] = 0;
                    } else if (++orphanStructures.plzPendingFrames[playerIndex] < plzDebounce) {
                        plzRoofFlipsHeld++;
                        continue;
                    }
                }
    ...
    public void doCutawayVisitSquares(int playerIndex, ArrayList<IsoChunk> onScreenChunks) {
        ...
        boolean plzPrefilter = PLZPerf.CUTAWAY_VISIT_PREFILTER;
        ...
        boolean plzChanged = PLZPerf.CUTAWAY_INVALIDATE_CHANGED && PerformanceSettings.fboRenderChunk;
        if (plzChanged) {
            this.plzOldFlags.clear();
            this.plzTouched.clear();
            ...
            for (int i = 0; i < perPlayerData1.lastCutawayVisitorResults.size(); i++) {
                ...
                if (this.plzOldFlags.put(square, square.getPlayerCutawayFlag(playerIndex, 0L)) == null) {
                    this.plzTouched.add(square);
                }
            }
        }
        ...
        for (int i = 0; i < perPlayerData1.lastCutawayVisitorResults.size(); i++) {
            ...
            if (!plzChanged && chunk != null && !this.invalidatedChunks.contains(chunk)) {
        ...
        if (plzChanged) {
            for (IsoGridSquare square : perPlayerData1.cutawayVisitorResultsNorth) {
                if (!this.plzOldFlags.containsKey(square)) {
                    this.plzOldFlags.put(square, square.getPlayerCutawayFlag(playerIndex, 0L));
                    this.plzTouched.add(square);
                }
            }
        ...
        if (plzChanged) {
            for (int i = 0; i < this.plzTouched.size(); i++) {
                IsoGridSquare square = this.plzTouched.get(i);
                int oldFlag = this.plzOldFlags.get(square);
                if (oldFlag != square.getPlayerCutawayFlag(playerIndex, 0L)) {
                    plzCutawayChangedSquares++;
                    IsoChunk chunk = square.getChunk();
            ...
            plzCutawayVisits++;
            plzCutawayChunksInvalidated = plzCutawayChunksInvalidated + this.invalidatedChunks.size();
            this.plzOldFlags.clear();
            this.plzTouched.clear();
        }
    ...
    private static boolean plzWallCanCut(FBORenderCutaways.CutawayWall wall, ArrayList<Long> roomIds, boolean peeking, ArrayList<BuildingDef> collapse) {
        if (roomIds.isEmpty()) {
            ...
            for (int i = 0; i < collapse.size(); i++) {
                if (wall.isPartOfBuilding(collapse.get(i))) {
                    return true;
                }
            }
            ...
            for (int i = 0; i < roomIds.size(); i++) {
                if (wall.occludedRoomIds.contains(roomIds.get(i))) {
                    return true;
                }
            }
    ...
    private void cutawayVisit(IsoGridSquare poiSquare, long currentTimeMillis, ArrayList<IsoChunk> onScreenChunks) {
        ...
        if (chunkMap != null && !chunkMap.ignore) {
            FBORenderCutaways.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
            boolean plzPrefilter = PLZPerf.CUTAWAY_VISIT_PREFILTER;
            ArrayList<Long> plzRoomIds = plzPrefilter ? this.cell.tempPlayerCutawayRoomIds.get(playerIndex) : null;
            boolean plzPeeking = plzPrefilter && this.cell.playerWindowPeekingRoomId[playerIndex] != -1L;
            ArrayList<BuildingDef> plzCollapse = plzPrefilter ? perPlayerData1.buildingsToCollapse.buildingsToCollapse : null;
            ...
            for (int j = 0; j < perPlayerData1.cutawayWalls.size(); j++) {
                ...
                if (plzPrefilter && !plzWallCanCut(wall, plzRoomIds, plzPeeking, plzCollapse)) {
                    plzWallsSkipped++;
                } else {
                    plzWallsVisited++;
                    if (wall.isHorizontal()) {
                        for (int x1 = wall.x1; x1 < wall.x2; x1++) {
                            ...
                            if (test != null) {
                                ...
                                FBORenderCutaways.ChunkLevelData levelData = test.chunk == wall.chunkLevelData.levelsData.chunk
                                    ? wall.chunkLevelData
                                    : test.chunk.getCutawayDataForLevel(level);
    ...
    public boolean CalculateBuildingsToCollapse() {
        ...
        if (changed) {
            StringBuilder sb = new StringBuilder("collapse list changed: ")
                .append(btc.tempLastBuildingsToCollapse.size())
                .append(" -> ")
                .append(btc.buildingsToCollapse.size())
                .append(" [");
            ...
            for (int i = 0; i < walls.size(); i++) {
                ...
                if (wall.isHorizontal()) {
                    for (int x = wall.x1; x < wall.x2; x++) {
                        ...
                        LineDrawer.addLine(
                            x + (x == wall.x1 ? 0.05F : 0.0F), wall.y1, this.level, x + 1 - (x == wall.x2 - 1 ? 0.05F : 0.0F), wall.y2, this.level, r, 1.0F, b, 1.0F
                        );
                    }
                } else {
                    for (int y = wall.y1; y < wall.y2; y++) {
                        ...
                        LineDrawer.addLine(
                            wall.x1, y + (y == wall.y1 ? 0.05F : 0.0F), this.level, wall.x1, y + 1 - (y == wall.y2 - 1 ? 0.05F : 0.0F), this.level, r, 1.0F, b, 1.0F
                        );
                    }
                }
            }
    ...
    public static final class OrphanStructures {
        FBORenderCutaways.ChunkLevelData chunkLevelData;
        final FBORenderCutaways.PlayerInRange[] playerInRange = new FBORenderCutaways.PlayerInRange[4];
        final int[] plzPendingFrames = new int[4];
        ...
        void calculate(IsoChunk chunk) {
            ...
            Arrays.fill(this.plzPendingFrames, 0);
