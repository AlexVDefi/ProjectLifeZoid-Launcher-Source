...
public final class FBORenderCell {
    ...
    private boolean plzTreesPerFrameNow;
    private int plzTreeModeFrame = -1;
    static long plzTreesPerFrameFrames;
    private final PLZTreeBake.Rect plzTreeRect = new PLZTreeBake.Rect();
    private final PLZTreeBake.Rect plzTreeOwnTexture = new PLZTreeBake.Rect();
    private final PLZTreeBake.Rect plzTreeOtherTexture = new PLZTreeBake.Rect();
    private final FBORenderCell.TreeBakePass plzTreePass = new FBORenderCell.TreeBakePass();

    /** A crown reaches at most two chunks in each direction, so the neighbourhood is 5x5. */
    private static final int PLZ_TREE_REACH = 2;
    private static final int PLZ_TREE_SPAN = PLZ_TREE_REACH * 2 + 1;
    private static final int PLZ_TREE_SLOTS = PLZ_TREE_SPAN * PLZ_TREE_SPAN;
    private static final int PLZ_TREE_OWN_SLOT = PLZ_TREE_SLOTS / 2;
    private final HashSet<IsoTree> plzTreesAwaitingTexture = new HashSet<>();
    private static long plzTreesWaited;
    private static long plzTreesArrived;
    private final PLZBakeScheduler plzBakes = new PLZBakeScheduler();
    private final ArrayList<IsoChunk> plzNearChunks = new ArrayList<>();
    ...
    public void renderInternal() {
        ...
        try (ProfileArea var24 = GameProfiler.getInstance().profile("renderLast")) {
    ...
    public void RenderTiles(int minHeight, int maxHeight) {
        ...
        try (AbstractPerformanceProfileProbe var3 = s_performance.isoCellRenderTiles.profile()) {
            this.renderTilesInternal(maxHeight);
        }
    }

    private void renderTilesInternal(int maxHeight) {
        this.plzUpdateTreeMode();
        ...
        if (DebugOptions.instance.terrain.renderTiles.enable.getValue()) {
            ...
            FBORenderLevels.clearCachedSquares = !plzKeepPerFrameLists();
            int playerIndex = IsoCamera.frameState.playerIndex;
            ...
            try (ProfileArea bForceCutawayUpdate = GameProfiler.getInstance().profile("updateWeatherMask")) {
            ...
            boolean var31;
            try (ProfileArea size = GameProfiler.getInstance().profile("runChecks")) {
                var31 = bForceCutawayUpdatex | this.runChecks(playerIndex);
            }

            boolean var32;
            try (AbstractPerformanceProfileProbe var39 = renderTiles.recalculateAnyGridStacks.profile()) {
                if (checkFuture != null) {
            ...
            try (AbstractPerformanceProfileProbe var41 = cutaways.profile()) {
                ...
                if (var38) {
                    FBORenderCutaways.getInstance().doCutawayVisitSquares(playerIndex, this.plzCutawayVisitChunks(perPlayerData1));
                }
            }

            perPlayerData1.occlusionChanged = false;
            if (FBORenderOcclusion.getInstance().enabled && this.plzHasDirtyChunkTexturesForOcclusion(playerIndex)) {
            ...
            try (AbstractPerformanceProfileProbe var44 = renderTiles.performRenderTiles.profile()) {
            ...
            try (AbstractPerformanceProfileProbe var45 = renderTiles.renderDebugPhysics.profile()) {
                this.cell.renderDebugPhysics(playerIndex);
            }

            try (AbstractPerformanceProfileProbe var46 = renderTiles.renderDebugLighting.profile()) {
    ...
    private boolean recalculateGridStacks(IsoPlayer player, int playerIndex) {
        ...
        int plzInterval = PLZPerf.GRID_STACK_INTERVAL;
        boolean plzSkip = false;
        if (plzInterval > 0 && !player.dirtyRecalcGridStack) {
            IsoGridSquare sq = IsoCamera.frameState.camCharacterSquare;
            IsoDirections dir = player.getDir();
            int frame = IsoWorld.instance.getFrameNo();
            plzSkip = sq != null && sq == this.plzGridStackSquare && dir == this.plzGridStackDir && frame - this.plzGridStackFrame < plzInterval;
            if (!plzSkip) {
                this.plzGridStackSquare = sq;
                this.plzGridStackDir = dir;
                this.plzGridStackFrame = frame;
            }
        }
                                        ...
                                        for (int spriteGridX = 0; spriteGridX < spriteGrid.getWidth(); spriteGridX++) {
                                            if (spriteGrid.getSprite(spriteGridX, spriteGridY) != null) {
                                                ...
                                                if (chunkMap.getGridSquare(squareX, squareY, squareZ) != null
                                                    ...
                                                    Location location1 = this.locationPool.alloc();
    ...
    private void testSquareObscuringPlayer(int playerIndex, int x, int y, int z, ArrayList<Location> locations) {
        ...
        if (this.isSquareObscuringPlayer(playerIndex, square)) {
            ...
            Location location = this.locationPool.alloc();
    ...
    private boolean checkTreeTranslucency(int playerIndex, FBORenderLevels renderLevels) {
        ...
        for (int i = 0; i < squares.size(); i++) {
            ...
            if (square.chunk != null) {
                ...
                if (tree != null && PLZPerf.TREES_IN_CHUNK_TEXTURE) {
                    if (!plzTreeTextureReady(tree)) {
                        if (this.plzTreesAwaitingTexture.add(tree)) {
                            plzTreesWaited++;
                        }
                    } else if (!this.plzTreesAwaitingTexture.isEmpty() && this.plzTreesAwaitingTexture.remove(tree)) {
                        plzTreesArrived++;
                        bChanged = true;
                        ...
                        this.plzInvalidateTreeCopies(tree, playerIndex);
                    }
                }
                ...
                if (tree != null && !this.isTreeRenderedEveryFrame(tree)) {
                    ...
                    if (this.plzIsTranslucentTree(tree, plzAiming) != tree.renderFlag) {
    ...
    private void performRenderTiles(PerPlayerRender perPlayerRender, int playerIndex, long currentTimeMillis) {
        ...
        for (int z = chunkMap.minHeight; z <= chunkMap.maxHeight; z++) {
            ...
            try (AbstractPerformanceProfileProbe var36 = puddles.profile()) {
            ...
            if (z == PZMath.fastfloor(IsoCamera.frameState.camCharacterZ)) {
                ...
                try (ProfileArea var36 = GameProfiler.getInstance().profile("Visibility")) {
    ...
    private void renderOneChunk(
        ...
        if (c != null && c.IsOnScreen(true) && !c.lightingNeverDone[playerIndex]) {
            FBORenderLevels renderLevels = c.getRenderLevels(playerIndex);
    ...
    private void renderOneLevel(
        ...
        if (!renderLevels.isOnScreen(level)) {
            ...
            if (PLZBakeScheduler.active() && renderLevels.isDirty(level, zoom)) {
                FBORenderChunk plzTexture = renderLevels.getFBOForLevel(level, zoom);
                boolean plzDefer;
                if (level == renderLevels.getMinLevel(level)) {
                    plzDefer = !this.plzBakes.admit(
                    ...
                    plzDefer = this.plzBakes.wasDeferred(plzTexture);
                }

                if (plzDefer) {
                    if (plzTexture != null && !renderLevels.isDirty(level, PLZBakeScheduler.DIRTY_CREATE, zoom)) {
                        FBORenderChunkManager.instance.renderChunk = plzTexture;
            ...
            if (isDirty && canRender) {
                if (level == renderLevels.getMinLevel(level)) {
                    this.plzBakes.recordBaked(FBORenderChunkManager.instance.renderChunk, currentTimeMillis);
                }
                ...
                try (AbstractPerformanceProfileProbe var17 = calculateRenderInfo.profile()) {
                    if (level == renderLevels.getMinLevel(level)) {
                        ...
                        PLZPuddleCache.invalidate(c, level);
                    }
                    ...
                    for (int i = 0; i < squares.length; i++) {
                        ...
                        if (levelData.shouldRenderSquare(playerIndex, square) && !FBORenderOcclusion.getInstance().isOccluded(square.x, square.y, square.z)) {
                            if (!square.getObjects().isEmpty()) {
                                ...
                                try (ProfileArea squarex = profiler.profile("Calculate")) {
                                ...
                                try (ProfileArea var85 = profiler.profile("Puddles")) {
                ...
                try (AbstractPerformanceProfileProbe var68 = renderOneChunkLevel2.profile()) {
                    if (level == renderLevels.getMinLevel(level)) {
                    ...
                    if (renderFloor) {
                        try (ProfileArea var59 = profiler.profile("Floor")) {
                        ...
                        try (ProfileArea var751 = profiler.profile("Snow")) {
                            IndieGL.disableDepthTest();
                            FBORenderSnow.getInstance().RenderSnow(c, level);
                            IndieGL.enableDepthTest();
                        }

                        try (ProfileArea var76 = profiler.profile("Blood")) {
                    ...
                    if (DebugOptions.instance.terrain.renderTiles.vegetationCorpses.getValue()) {
                        try (ProfileArea var62 = profiler.profile("Vegetation Corpses")) {
                            if (DebugOptions.instance.fboRenderChunk.corpsesInChunkTexture.getValue()) {
                    ...
                    if (DebugOptions.instance.terrain.renderTiles.minusFloorCharacters.getValue()) {
                        try (ProfileArea var63 = profiler.profile("Minus Floor Chars")) {
                            if (DebugOptions.instance.fboRenderChunk.itemsInChunkTexture.getValue()) {
                                IsoGridSquare squareNW = c.getGridSquare(0, 0, level);
                                IsoGridSquare squareN = squareNW == null ? null : squareNW.getAdjacentSquare(IsoDirections.N);
                                if (squareNW != null && squareN != null) {
                    ...
                    if (PerformanceSettings.puddlesQuality == 2) {
                        try (ProfileArea var64 = profiler.profile("Low Puddles")) {
                            this.renderPuddlesToChunkTexture(playerIndex, level, c);
                        }
                    }

                    try (ProfileArea var65 = profiler.profile("Add Chunk")) {
                        if (!renderLevels.getCachedSquares_AnimatedAttachments(level).isEmpty()) {
                            perPlayerData1.addChunkWith_AnimatedAttachments(c);
                        }
                ...
                if (this.plzTreePassActive()
                    && FBORenderChunkManager.instance.renderChunk != null
                    && FBORenderChunkManager.instance.renderChunk.isTopLevel(level)) {
                    this.plzBakeTrees(c, playerIndex, zoom);
                }
    ...
    private ObjectRenderLayer calculateObjectRenderLayer(IsoObject object) {
        if (!object.getDoRender()) {
            ...
            ObjectRenderLayer plzCurtainLayer = this.plzCurtainLayer(object);
            if (plzCurtainLayer != null) {
                return plzCurtainLayer;
                ...
                return plzPerFrameTranslucentTile(object.getSprite()) ? ObjectRenderLayer.TranslucentSE : ObjectRenderLayer.MinusFloorSE;
            } else {
    ...
    private ObjectRenderLayer plzCurtainLayer(IsoObject object) {
        if (!(object instanceof IsoCurtain curtain)) {
            return null;
        } else {
            if (!PLZPerf.WINDOWS_IN_CHUNK_TEXTURE && !PLZPerf.TRANSLUCENT_TILES_IN_CHUNK_TEXTURE) {
                return null;
            }
            ...
            if ((type == IsoObjectType.curtainN || type == IsoObjectType.curtainW) && curtain.getSpriteModel() == null) {
                ...
                if (attached != null && attached.square == object.square) {
                    ...
                    return layer != ObjectRenderLayer.MinusFloor && layer != ObjectRenderLayer.Translucent ? null : layer;
                } else {
                    return null;
                }
            } else {
    ...
    private boolean isObjectRenderLayer_MinusFloor(IsoObject object) {
        IsoSprite sprite = object.getSprite();
        if (plzPerFrameTranslucentTile(sprite)) {
            return false;
        }
        ...
        if (object.getObjectRenderEffectsToApply() != null) {
            ...
            boolean plzBake = PLZPerf.WINDOWS_IN_CHUNK_TEXTURE;
            boolean bTranslucent = !plzBake && object instanceof IsoWindow;
            IsoDoor door = Type.tryCastTo(object, IsoDoor.class);
            bTranslucent |= !plzBake && door != null && door.getProperties() != null && door.getProperties().has("doorTrans");
            int playerIndex = IsoCamera.frameState.playerIndex;
            FBORenderCell.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
    ...
    private static boolean plzPerFrameTranslucentTile(IsoSprite sprite) {
        return sprite != null && (sprite.depthFlags & 2) != 0 && (!PLZPerf.TRANSLUCENT_TILES_IN_CHUNK_TEXTURE);
    }
    ...
    private boolean plzTreePassActive() {
        return PLZPerf.TREES_IN_CHUNK_TEXTURE
            && PLZPerf.TREE_BAKE_PASS
            && !this.plzTreesPerFrameNow
            && !Core.getInstance().getOptionDoWindSpriteEffects();
    }

    private void plzUpdateTreeMode() {
        int frame = IsoWorld.instance.getFrameNo();
        if (frame != this.plzTreeModeFrame) {
            this.plzTreeModeFrame = frame;
            int max = PLZPerf.TREE_BAKE_MAX_CHUNKS_PER_SEC;
            this.plzTreesPerFrameNow = max > 0 && PLZChunkRate.perSecond() > max;
            if (this.plzTreesPerFrameNow) {
                plzTreesPerFrameFrames++;
            }
        }
    }

    private boolean plzTreeBakes(IsoTree tree, boolean aiming) {
        if (tree.square == null || tree.square.z >= 1000 || tree.isHighlighted() || tree.isAnimating()) {
            return false;
        } else {
            return tree.getObjectRenderEffects() == null && tree.getObjectRenderEffectsToApply() == null && !(tree.fadeAlpha < 1.0F)
                ? !this.isTreeRenderedEveryFrame(tree) && !this.plzIsTranslucentTree(tree, aiming)
                : false;
        }
    }

    private boolean plzTreeRect(IsoTree tree, int wx, int wy, float goX, float yoff, PLZTreeBake.Rect out) {
        ...
        float scale = PLZTreeBake.spriteScale(tileScale, texture.getWidthOrig(), texture.getHeightOrig());
        IsoGridSquare square = tree.square;
        PLZTreeBake.spriteRect(
            square.x,
            square.y,
            square.z,
            wx,
            wy,
            PLZTreeBake.offsetX(sprite.name, tileScale),
            PLZTreeBake.offsetY(sprite.name, tileScale),
            texture.getWidthOrig() * scale,
            texture.getHeightOrig() * scale,
            goX,
            yoff,
            tileScale,
            out
    ...
    private static float plzTextureYOffset(int minLevel, int topLevel) {
        return (topLevel - minLevel + 1) * FBORenderChunk.PIXELS_PER_LEVEL
            + minLevel * FBORenderChunk.PIXELS_PER_LEVEL
            + FBORenderLevels.extraHeightForJumboTrees(minLevel, topLevel);
    }

    private static boolean plzChunkTextureRect(IsoChunk n, IsoChunk c, int minLevel, float zoom, float goX, float yoff, PLZTreeBake.Rect out) {
        if (minLevel >= 0 && minLevel >= n.minLevel && minLevel <= n.maxLevel) {
            ...
            PLZTreeBake.neighbourTextureRect(n.wx - c.wx, n.wy - c.wy, goX, yoff, plzTextureYOffset(minLevel, topLevel), w, h, Core.tileScale, out);
            return true;
        } else {
            return false;
        }
    ...
    private static void plzInvalidateTreeHolder(IsoChunk n, int minLevel, int playerIndex, float zoom) {
    ...
    private void plzInvalidateTreeCopies(IsoTree tree, int playerIndex) {
        if (this.plzTreePassActive() && tree.square != null && tree.square.chunk != null) {
            IsoChunk c = tree.square.chunk;
            int minLevel = FBORenderLevels.calculateMinLevel(tree.square.z);
            float zoom = Core.getInstance().getZoom(playerIndex);
            PLZTreeBake.Rect sprite = this.plzTreeRect;
            PLZTreeBake.Rect own = this.plzTreeOwnTexture;
            PLZTreeBake.Rect other = this.plzTreeOtherTexture;
            float goX = 0.0F;
            float yoff = 0.0F;
            if (plzChunkTextureRect(c, c, minLevel, zoom, goX, yoff, own) && this.plzTreeRect(tree, c.wx, c.wy, goX, yoff, sprite)) {
                for (int dwy = -2; dwy <= 2; dwy++) {
                    for (int dwx = -2; dwx <= 2; dwx++) {
                        if (dwx != 0 || dwy != 0) {
                            IsoChunk n = this.cell.getChunk(c.wx + dwx, c.wy + dwy);
                            if (n != null && plzChunkTextureRect(n, c, minLevel, zoom, goX, yoff, other) && PLZTreeBake.needsCopy(sprite, other, own)) {
                                plzInvalidateTreeHolder(n, minLevel, playerIndex, zoom);
                            }
                        }
                    }
                }
    ...
    /**
     * Draws every tree whose crown lands on this chunk's texture into it, including trees standing
     * on neighbouring chunks. Runs once per texture bake, not per frame.
     *
     * A crown is wider and taller than its own square - a JUMBOXXL reaches seven tiles - so the
     * neighbourhood searched is 5x5 chunks, and the reverse is also true: a tree on THIS chunk
     * exports into neighbours, which therefore have to re-bake when it changes. That bookkeeping
     * is the fingerprint pass at the end.
     */
    private void plzBakeTrees(IsoChunk c, int playerIndex, float zoom) {
        ...
        FBORenderCell.TreeBakePass pass = this.plzTreePass;
        pass.begin(c, rc, playerIndex, zoom, minLevel);

        for (int slot = 0; slot < PLZ_TREE_SLOTS; slot++) {
            IsoChunk source = pass.sourceAt(slot);
            if (source != null && pass.sourceIsUsable(slot, source)) {
                this.plzBakeTreesFrom(pass, slot, source);
            }
        }

        pass.reconcileExports();
        pass.finish();
    }

    /** One neighbourhood slot's contribution: every tree on it that reaches this texture. */
    private void plzBakeTreesFrom(FBORenderCell.TreeBakePass pass, int slot, IsoChunk source) {
        boolean own = slot == PLZ_TREE_OWN_SLOT;
        int from = Math.max(pass.minLevel, source.minLevel);
        int to = Math.min(pass.topLevel, source.maxLevel);

        for (int z = from; z <= to; z++) {
            IsoGridSquare[] squares = source.squares[source.squaresIndexOfLevel(z)];
            FBORenderCutaways.ChunkLevelData levelData = source.getCutawayDataForLevel(z);
            ...
            for (int i = 0; i < squares.length; i++) {
                ...
                IsoTree tree = square.getTree();
                if (tree == null || !this.plzTreeIsBakeable(pass, tree, square, levelData, own)) {
                    continue;
                }

                if (!this.plzTreeReachesTexture(pass, tree, own)) {
                    continue;
                }

                if (!own) {
                    // A neighbour's square is not lit as part of this chunk's own pass.
                    square.cacheLightInfo();
                }

                this.plzDrawBakedTree(pass, tree, square, z);
            }
        }
    }

    /** The eligibility test, separated from the geometry test so each reads as one thing. */
    private boolean plzTreeIsBakeable(
        FBORenderCell.TreeBakePass pass, IsoTree tree, IsoGridSquare square, FBORenderCutaways.ChunkLevelData levelData, boolean own
    ) {
        return this.plzTreeBakes(tree, pass.aiming)
            && (!own || tree.getRenderInfo(pass.playerIndex).layer == ObjectRenderLayer.MinusFloor)
            && levelData.shouldRenderSquare(pass.playerIndex, square)
            && !FBORenderOcclusion.getInstance().isOccluded(square.x, square.y, square.z)
            && !FBORenderCutaways.getInstance().isForceRenderSquare(pass.playerIndex, square);
    }

    /**
     * Places the crown and decides whether this texture actually gets any of it. For a tree on
     * this chunk it also records which neighbours the crown spills into, so they can be re-baked.
     */
    private boolean plzTreeReachesTexture(FBORenderCell.TreeBakePass pass, IsoTree tree, boolean own) {
        if (!this.plzTreeRect(tree, pass.chunk.wx, pass.chunk.wy, pass.goX, pass.yoff, this.plzTreeRect)) {
            return false;
        }

        if (!own) {
            return PLZTreeBake.needsCopy(this.plzTreeRect, this.plzTreeOwnTexture, this.plzTreeOtherTexture);
        }

        pass.recordExports(this.plzTreeRect, this.plzTreeOwnTexture, this.plzTreeOtherTexture, tree);
        // Drawn into the texture rather than per frame from here on, whether or not it lands here.
        tree.renderFlag = false;
        return PLZTreeBake.overlaps(this.plzTreeRect, this.plzTreeOwnTexture);
    }

    /** Depth rises up the crown, so an upper floor behind the tree no longer slices through it. */
    private void plzDrawBakedTree(FBORenderCell.TreeBakePass pass, IsoTree tree, IsoGridSquare square, int z) {
        if (square.getLightInfo(pass.playerIndex) == null) {
            return;
        }

        float base = IsoDepthHelper.getSquareDepthData(pass.camX, pass.camY, square.x + 0.99F, square.y + 0.99F, z).depthStart
            - IsoDepthHelper.getChunkDepthData(
                PZMath.fastfloor(pass.camX / 8.0F), PZMath.fastfloor(pass.camY / 8.0F), pass.chunk.wx, pass.chunk.wy, z).depthStart;
        if (base < 0.0F) {
            return;
        }

        ColorInfo light = this.sanitizeLightInfo(pass.playerIndex, square);
        boolean unlit = tree.getSprite().getProperties().has(IsoFlagType.unlit);
        float r = unlit ? 1.0F : light.r;
        float g = unlit ? 1.0F : light.g;
        float b = unlit ? 1.0F : light.b;

        PLZTreeBake.Drawer drawer = pass.drawer();
        IsoDirections dir = tree.getForwardIsoDirection();
        PLZTreeBake.Rect sprite = this.plzTreeRect;
        this.plzAddTreeTexture(drawer, tree.getSprite().getTextureForCurrentFrame(dir, tree),
            sprite.x0, sprite.y0, sprite.ground, base, r, g, b, pass.tileScale);
        ...
        for (int k = 0; k < tree.attachedAnimSprite.size(); k++) {
            IsoSpriteInstance attached = tree.attachedAnimSprite.get(k);
            this.plzAddTreeTexture(drawer, attached.parentSprite.getTextureForCurrentFrame(dir, tree),
                sprite.x0, sprite.y0, sprite.ground, base, r, g, b, pass.tileScale);
        }
    }

    /**
     * Per-bake state for the tree pass, so the walk itself reads as a walk.
     *
     * Reused between bakes - a bake happens often enough that allocating the neighbourhood arrays
     * each time is measurable, and it is single-threaded per cell.
     */
    private final class TreeBakePass {
        private final IsoChunk[] sources = new IsoChunk[PLZ_TREE_SLOTS];
        private final int[] exports = new int[PLZ_TREE_SLOTS];
        ...
        private PLZTreeBake.Drawer drawer;

        void begin(IsoChunk c, FBORenderChunk rc, int playerIndex, float zoom, int minLevel) {
            this.chunk = c;
            this.playerIndex = playerIndex;
            this.zoom = zoom;
            this.minLevel = minLevel;
            this.topLevel = rc.getTopLevel();
            this.goX = FBORenderChunkManager.instance.getXOffset();
            this.yoff = FBORenderChunkManager.instance.getYOffset();
            ...
            PLZTreeBake.ownTextureRect(this.goX, rc.w / scale, rc.h / scale, FBORenderCell.this.plzTreeOwnTexture);

            for (int slot = 0; slot < PLZ_TREE_SLOTS; slot++) {
                this.sources[slot] = slot == PLZ_TREE_OWN_SLOT
                    ? c
                    : FBORenderCell.this.cell.getChunk(
                        c.wx + slot % PLZ_TREE_SPAN - PLZ_TREE_REACH, c.wy + slot / PLZ_TREE_SPAN - PLZ_TREE_REACH);
            }
        }

        IsoChunk sourceAt(int slot) {
            return this.sources[slot];
        }

        /** A neighbour only matters if it has been lit and its texture rect can be placed. */
        boolean sourceIsUsable(int slot, IsoChunk source) {
            if (slot == PLZ_TREE_OWN_SLOT) {
                return true;
            }
            return !source.lightingNeverDone[this.playerIndex]
                && plzChunkTextureRect(source, this.chunk, this.minLevel, this.zoom, this.goX, this.yoff,
                    FBORenderCell.this.plzTreeOtherTexture);
        }

        /**
         * Notes every neighbour this crown spills into. Order-sensitive mix rather than a sum:
         * two trees swapping places must change the fingerprint, and a plain sum would not.
         */
        void recordExports(PLZTreeBake.Rect sprite, PLZTreeBake.Rect own, PLZTreeBake.Rect other, IsoTree tree) {
            int id = System.identityHashCode(tree);
            for (int slot = 0; slot < PLZ_TREE_SLOTS; slot++) {
                if (slot == PLZ_TREE_OWN_SLOT || this.sources[slot] == null) {
                    continue;
                }
                if (plzChunkTextureRect(this.sources[slot], this.chunk, this.minLevel, this.zoom, this.goX, this.yoff, other)
                    && PLZTreeBake.needsCopy(sprite, other, own)) {
                    this.exports[slot] = this.exports[slot] * 31 + id;
                }
            }
        }

        /** Re-bakes the neighbours whose share of this chunk's trees changed since last time. */
        void reconcileExports() {
            int[] last = this.chunk.plzTreeExportFp;
            if (last == null) {
                last = this.chunk.plzTreeExportFp = new int[PLZ_TREE_SLOTS];
            }

            for (int slot = 0; slot < PLZ_TREE_SLOTS; slot++) {
                if (this.exports[slot] == last[slot]) {
                    continue;
                }

                last[slot] = this.exports[slot];
                IsoChunk neighbour = this.sources[slot];
                if (neighbour != null && neighbour != this.chunk
                    && this.minLevel >= neighbour.minLevel && this.minLevel <= neighbour.maxLevel) {
                    plzInvalidateTreeHolder(neighbour, this.minLevel, this.playerIndex, this.zoom);
                }
            }
        }

        PLZTreeBake.Drawer drawer() {
            if (this.drawer == null) {
                this.drawer = PLZTreeBake.alloc();
            }
            return this.drawer;
        }
        ...
        void finish() {
            if (this.drawer != null) {
                SpriteRenderer.instance.drawGeneric(this.drawer);
                this.drawer = null;
            }
    ...
    private void plzAddTreeTexture(
        PLZTreeBake.Drawer drawer, Texture texture, float sx, float sy, float ground, float base, float r, float g, float b, int tileScale
    ) {
        if (texture != null && texture.isReady() && texture.getTextureId() != null) {
            ...
            drawer.add(texture, x0, y0, x1, y1, PLZTreeBake.depthAtRow(base, ground, y0, tileScale), PLZTreeBake.depthAtRow(base, ground, y1, tileScale), r, g, b, 1.0F);
        }
    }

    private static boolean plzIsJumboTree(IsoObject object) {
        IsoSprite sprite = object.getSprite();
        if (sprite == null) {
            return false;
        }
    ...
    private static boolean plzTreeTextureReady(IsoObject object) {
        IsoSprite sprite = object.getSprite();
        if (sprite == null) {
            return true;
        }
    ...
    private boolean plzIsTranslucentTree(IsoObject object, boolean isAiming) {
        IsoGridSquare square = object.square;
        square.IsOnScreen();
        if (!isAiming
            && (
    ...
    public void renderFloor(IsoObject object) {
        ...
        object.setAlpha(playerIndex, renderInfo.targetAlpha);
        if (DebugOptions.instance.terrain.renderTiles.renderGridSquares.getValue() && object.sprite != null) {
    ...
    private void renderMinusFloor(IsoChunk chunk, IsoGridSquare square, PZArrayList<IsoObject> objectList) {
        ...
        IsoObject[] objects = objectList.getElements();
        int numObjects = objectList.size();
        boolean plzTreePass = this.plzTreePassActive();

        for (int i = 0; i < numObjects; i++) {
            IsoObject object = objects[i];
            ObjectRenderInfo renderInfo = object.getRenderInfo(playerIndex);
            if (renderInfo.layer == ObjectRenderLayer.MinusFloor && (!plzTreePass || !(object instanceof IsoTree))) {
    ...
    private void renderMinusFloor(IsoObject object) {
        if (object instanceof IsoTree && PLZPerf.TREES_IN_CHUNK_TEXTURE && PLZPerf.TREE_BAKE_DIRECT) {
            FBORenderTrees batch = FBORenderTrees.current;
            FBORenderTrees.current = null;

            try {
                this.plzRenderMinusFloorInner(object);
            } finally {
    ...
    private void plzRenderMinusFloorInner(IsoObject object) {
        int playerIndex = IsoCamera.frameState.playerIndex;
        ObjectRenderInfo renderInfo = object.getRenderInfo(playerIndex);
        IsoObjectType t = IsoObjectType.MAX;
        if (object.sprite != null) {
    ...
    void renderMinusFloor_NotDoorOrWall(IsoObject object) {
        ...
        if (!this.renderTranslucentOnly && object instanceof IsoCurtain curtain && plzCurtainDepthNudge(curtain) > 0.0F) {
            plzRenderCurtainNudged(curtain, square, lightInfo, plzCurtainDepthNudge(curtain));
        } else {
            object.render(square.x, square.y, square.z, lightInfo, true, false, null);
        }
    }

    private static float plzCurtainDepthNudge(IsoCurtain curtain) {
        if (PLZPerf.CURTAIN_DEPTH_NUDGE <= 0.0F) {
            return 0.0F;
        } else if (!PLZPerf.WINDOWS_IN_CHUNK_TEXTURE && !PLZPerf.TRANSLUCENT_TILES_IN_CHUNK_TEXTURE) {
            return 0.0F;
        } else {
            IsoObjectType type = curtain.getType();
            if (type != IsoObjectType.curtainN && type != IsoObjectType.curtainW) {
                ...
                return curtain.getSpriteModel() != null ? 0.0F : PLZPerf.CURTAIN_DEPTH_NUDGE;
            }
        }
    }

    private static void plzRenderCurtainNudged(IsoCurtain curtain, IsoGridSquare square, ColorInfo lightInfo, float nudge) {
        ...
        curtain.offsetX = curtain.offsetX + IsoUtils.XToScreen(dx, dy, 0.0F, 0);
        curtain.offsetY = curtain.offsetY + IsoUtils.YToScreen(dx, dy, 0.0F, 0);
    ...
    private void renderWorldInventoryObjects(IsoGridSquare square, IsoGridSquare renderSquare, boolean bChunkTexture) {
        if (this.shouldRenderSquare(renderSquare)) {
            ...
            this.timSort.doSort(this.tempWorldInventoryObjects.getElements(), (Comparator<IsoWorldInventoryObject>)(o1, o2) -> {
    ...
    /** A deferred texture re-reads its cached square lists next frame, so they cannot be cleared. */
    private static boolean plzKeepPerFrameLists() {
        return PLZBakeScheduler.active();
    }
    ...
    public void renderTranslucent(IsoObject object) {
        ...
        if (sprite != null && sprite.getProperties().has(IsoFlagType.transparentFloor)) {
            ...
            if ((!(object instanceof IsoBarbecue) || !FBORenderObjectHighlight.getInstance().isRendering()) && object.hasAnimatedAttachments()) {
    ...
    private void updateChunkLighting(int playerIndex) {
        if (!DebugOptions.instance.fboRenderChunk.nolighting.getValue() && DebugOptions.instance.fboRenderChunk.updateSquareLightInfo.getValue()) {
            FBORenderCell.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
            int plzLightBudget = PLZPerf.LIGHTING_BUDGET;
            if (plzLightBudget > 0) {
                this.plzUpdateChunkLightingBudgeted(playerIndex, perPlayerData1, plzLightBudget);
                return;
            }
    ...
    private void plzUpdateChunkLightingBudgeted(int playerIndex, FBORenderCell.PerPlayerData perPlayerData1, int budget) {
        if (perPlayerData1.lightingUpdateCounter != LightingJNI.getUpdateCounter(playerIndex)) {
            perPlayerData1.lightingUpdateCounter = LightingJNI.getUpdateCounter(playerIndex);
    ...
    /** @return false for work that no longer applies, so it does not spend the frame's budget. */
    private boolean plzRefreshQueuedLevel(int playerIndex, FBORenderCell.PerPlayerData perPlayerData1, IsoChunk chunk, int level) {
        return perPlayerData1.onScreenChunks.contains(chunk) && this.plzCacheChunkLevelLightInfo(playerIndex, chunk, level);
    }

    private boolean plzIsChunkLevelLightingDirty(int playerIndex, IsoChunk chunk, int level) {
    ...
    private static void plzCacheLightInfo(int playerIndex, IsoChunk chunk, int level, int index, IsoGridSquare square) {
        if (PLZPerf.LIGHT_INFO_ONCE_PER_FRAME && level >= -32 && level < 32) {
            int[][] stamps = chunk.plzLightInfoFrame;
            if (stamps == null) {
                stamps = new int[256][];
                chunk.plzLightInfoFrame = stamps;
            }
    ...
    private boolean plzCacheChunkLevelLightInfo(int playerIndex, IsoChunk chunk, int level) {
        if (level >= chunk.minLevel && level <= chunk.maxLevel) {
            FBORenderCutaways.ChunkLevelData levelData = chunk.getCutawayDataForLevel(level);
            ...
            for (int i = 0; i < squares.length; i++) {
                ...
                if (square != null && levelData.shouldRenderSquare(playerIndex, square)) {
                    plzCacheLightInfo(playerIndex, chunk, level, i, square);
                }
            }
    ...
    private boolean plzHasDirtyChunkTexturesForOcclusion(int playerIndex) {
        if (PLZPerf.OCCLUSION_SKIP_LIGHTING_ONLY) {
            float zoom = Core.getInstance().getZoom(playerIndex);
            FBORenderCell.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
            ...
            for (int i = 0; i < perPlayerData1.onScreenChunks.size(); i++) {
                ...
                for (int z = c.minLevel; z <= c.maxLevel; z++) {
                    if (renderLevels.isOnScreen(z) && renderLevels.isDirty(z, zoom)) {
                        if (renderLevels.isDirty(z, -33L, zoom)) {
                            return true;
                        }
    ...
    private ArrayList<IsoChunk> plzCutawayVisitChunks(FBORenderCell.PerPlayerData perPlayerData1) {
        int r = PLZPerf.CUTAWAY_RADIUS;
        if (r <= 0) {
        ...
        this.plzNearChunks.clear();

        for (int i = 0; i < perPlayerData1.onScreenChunks.size(); i++) {
            IsoChunk c = perPlayerData1.onScreenChunks.get(i);
            if (Math.abs(c.wx - cx) <= r && Math.abs(c.wy - cy) <= r) {
                this.plzNearChunks.add(c);
            }
        }

        return this.plzNearChunks;
    }

    private void calculateOccludingSquares(int playerIndex) {
        FBORenderCell.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
        boolean plzFast = PLZPerf.CUTAWAY_FAST;
        float plzZoom = plzFast ? Core.getInstance().getZoom(playerIndex) : 0.0F;
        int plzWidth = perPlayerData1.occludedGridX2 - perPlayerData1.occludedGridX1 + 1;
        ...
        for (int i = 0; i < perPlayerData1.onScreenChunks.size(); i++) {
            ...
            for (int z = c.minLevel; z <= c.maxLevel; z++) {
                if (renderLevels.isOnScreen(z)) {
                    int plzSlot = z + 32;
                    if (plzFast && (c.plzOccluderMaskSet & 1L << plzSlot) != 0L && !renderLevels.isDirty(z, plzZoom)) {
                        long mask = c.plzOccluderMask[plzSlot];
                        ...
                        while (mask != 0L) {
                            ...
                            if (zeroX >= 0 && zeroY >= 0 && zeroX < plzWidth && zeroY <= perPlayerData1.occludedGridY2 - perPlayerData1.occludedGridY1) {
                                int idx = zeroX + zeroY * plzWidth;
                        ...
                        if (plzFast) {
                            c.plzOccluderMask[plzSlot] = plzExactOccluderMask(playerIndex, c, z, levelData);
                            c.plzOccluderMaskSet |= 1L << plzSlot;
                        }
    ...
    private static long plzExactOccluderMask(int playerIndex, IsoChunk chunk, int level, FBORenderCutaways.ChunkLevelData levelData) {
        ...
        for (int i = 0; i < 64; i++) {
            IsoGridSquare square = squares[i];
            if (square != null
                && levelData.shouldRenderSquare(playerIndex, square)
        ...
        if (chunk != null) {
            ...
            if (renderLevels.isOnScreen(z)) {
                ...
                for (int z2 = minLevel; z2 <= maxLevel; z2++) {
                    ...
                    boolean plzRefresh = !PLZPerf.LIGHT_INFO_CHUNK_GATE
                        || LightingJNI.getChunkDirty(playerIndex, chunk.wx, chunk.wy, z2 + 32);
                    if (!plzRefresh) {
                        plzLightInfoLevelsGated++;
                    }
                    ...
                    for (int y = 0; y < 8; y++) {
                        for (int x = 0; x < 8; x++) {
                            ...
                            if (sq != null) {
                                if (plzRefresh || sq.getLightInfo(playerIndex) == null) {
                                    plzCacheLightInfo(playerIndex, chunk, z2, x + y * 8, sq);
                                }
    ...
    private void renderOneLevel_Translucent(int playerIndex, IsoChunk c, int level) {
        ...
        if (renderLevels.isOnScreen(level)) {
            ...
            if (squaresObjects.size() + squaresItems.size() + squaresCutawayOutlines.size() != 0) {
                ...
                try (ProfileArea i = GameProfiler.getInstance().profile("Sort")) {
                ...
                this.timSort.doSort(sorted.getElements(), (Comparator<IsoGridSquare>)(o1, o2) -> {
    ...
    private void renderCorpseShadows(int playerIndex) {
        if (DebugOptions.instance.terrain.renderTiles.shadows.getValue() && Core.getInstance().getOptionCorpseShadows()) {
            FBORenderCell.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
    ...
    private void renderPlayer(int playerIndex, IsoPlayer player) {
        if (this.cell.getObjectList().contains(player)
            ...
            && player.isOnScreen()
            && player.getCurrentSquare().getLightInfo(playerIndex) != null
            && FBORenderCutaways.getInstance().shouldRenderBuildingSquare(playerIndex, player.getCurrentSquare())) {
    ...
    private void renderMovingObject(IsoMovingObject isoMovingObject) {
        ...
        if (isoMovingObject.getClass() != IsoPlayer.class
            ...
            && isoMovingObject.isOnScreen()
            && isoMovingObject.getCurrentSquare().getLightInfo(playerIndex) != null
            && this.shouldRenderSquare(isoMovingObject.getCurrentSquare())) {
            if (DebugOptions.instance.terrain.renderTiles.shadows.getValue()) {
    ...
    private void renderWater(int playerIndex) {
        if (DebugOptions.instance.weather.waterPuddles.getValue()
            ...
            && !(IsoCamera.frameState.camCharacterZ < 0.0F)) {
            FBORenderCell.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
            IsoChunkMap chunkMap = this.cell.chunkMap[playerIndex];
    ...
    private void renderPuddles(int playerIndex) {
        if (IsoPuddles.getInstance().shouldRenderPuddles() && !FBORenderSnow.getInstance().isSnowAnywhere()) {
            IsoChunkMap chunkMap = this.cell.chunkMap[playerIndex];
            int maxZ = chunkMap.maxHeight;
            ...
            FBORenderCell.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
            if (PLZPuddleCache.enabled()) {
                PLZPuddleCache.render(playerIndex, perPlayerData1.onScreenChunks, maxZ);
                return;
            }
    ...
    private void renderPuddlesTranslucentFloorsOnly(int playerIndex, int z) {
        if (IsoPuddles.getInstance().shouldRenderPuddles() && (Core.getInstance().getPerfPuddles() <= 0 || z == 0)) {
            FBORenderCell.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
            this.waterSquares.clear();
    ...
    private void renderPuddlesToChunkTexture(int playerIndex, int z, IsoChunk chunk) {
        if (IsoPuddles.getInstance().shouldRenderPuddles()
            ...
            && z >= chunk.minLevel
            && z <= chunk.maxLevel
            && chunk.getRenderLevels(playerIndex).isOnScreen(z)) {
            this.waterSquares.clear();
            FBORenderCutaways.ChunkLevelData levelData = chunk.getCutawayDataForLevel(z);
            IsoGridSquare[] squares = chunk.squares[chunk.squaresIndexOfLevel(z)];
    ...
    private void renderFog(int playerIndex) {
        if (!(IsoCamera.frameState.camCharacterZ < 0.0F) && PerformanceSettings.fogQuality != 2) {
            FBORenderCell.PerPlayerData perPlayerData1 = this.perPlayerData[playerIndex];
                                    ...
                                    if (levelData.shouldRenderSquare(playerIndex, square)) {
                                        ...
                                        for (int k = 0; k < numObjects; k++) {
                                            ...
                                            if (renderInfo.layer == ObjectRenderLayer.MinusFloor) {
                                                try (ProfileArea var17 = GameProfiler.getInstance().profile("ImprovedFog")) {
    ...
    private void debugChunkStateRenderPlayer(IsoPlayer player) {
        if (GameWindow.states.current == DebugChunkState.instance) {
            ...
            if (DebugChunkState.instance.getBoolean("ObjectAtCursor") && "player".equals(DebugChunkState.instance.fromLua1("getObjectAtCursor", "id"))) {
    ...
    public void renderSeamFix2_Floor(IsoObject object, float x, float y, float z, ColorInfo stCol, Consumer<TextureDraw> texdModifier) {
        if (!this.renderTranslucentOnly && PerformanceSettings.fboRenderChunk && DebugOptions.instance.fboRenderChunk.seamFix2.getValue()) {
            IsoGridSquare square = object.getSquare();
            IsoSprite sprite = object.getSprite();
            IsoGridSquare squareS = square.getAdjacentSquare(IsoDirections.S);
    ...
    public void renderSeamFix2_Wall(IsoObject object, float x, float y, float z, ColorInfo stCol, Consumer<TextureDraw> texdModifier) {
        if (PerformanceSettings.fboRenderChunk && DebugOptions.instance.fboRenderChunk.seamFix2.getValue()) {
            ...
            if (!sprite.getProperties().has(IsoFlagType.HoppableN)
                ...
                && (sprite.tileSheetIndex < 80 || sprite.tileSheetIndex > 82 || sprite.tilesetName == null || !sprite.tilesetName.equals("carpentry_02"))
                && (sprite.tileSheetIndex < 48 || sprite.tileSheetIndex > 55 || sprite.tilesetName == null || !sprite.tilesetName.equals("walls_logs"))
