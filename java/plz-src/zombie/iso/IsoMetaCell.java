...
import zombie.plz.PLZLotHeaders;
...
public final class IsoMetaCell {
    ...
    private PLZLotHeaders.Cache plzLotHeaders;
    ...
    private PLZLotHeaders.Cache plzLotHeaderCache() {
        if (this.plzLotHeaders == null || this.plzLotHeaders.header != this.info) {
            this.plzLotHeaders = new PLZLotHeaders.Cache(this.info);
        }

        return this.plzLotHeaders;
    }
    ...
    public IsoMetaChunk getChunk(int i) {
        if (!this.hasChunk(i)) {
            ...
            int chunkX = i % 32;
            int chunkY = i / 32;
            int intensity = PLZPerf.LOADER_CPU_FIXES
                ? PLZLotHeaders.zombieIntensity(this.info, chunkX, chunkY, this.plzLotHeaderCache())
                : LotHeader.getZombieIntensityForChunk(this.info, chunkX, chunkY);
