...
public final class IsoGridSquare {
    ...
    public boolean CalculateCollide(
        ...
        if (gridSquare != null && PLZBarrierGrid.isBlocked(gridSquare.x, gridSquare.y)) {
            return true;
        }
    ...
    public void RecalcProperties() {
        ...
        if (PLZBarrierGrid.isBlocked(this.x, this.y)) {
            this.properties.set(IsoFlagType.solidtrans);
        }
    ...
    public static void plzBarrierAddSegment(double x1, double y1, double x2, double y2) {
        PLZBarrierGrid.addSegment(x1, y1, x2, y2);
    }

    public static int plzBarrierEndUpdate() {
        return PLZBarrierGrid.endUpdate();
    }

    public static boolean plzBarrierIsBlocked(double x, double y) {
        return PLZBarrierGrid.isBlocked((int)Math.floor(x), (int)Math.floor(y));
    }

    public static int plzBarrierTileCount() {
        return PLZBarrierGrid.getTileCount();
    }

    public static String plzBarrierPatchStatus() {
        return PLZBarrierGrid.patchStatus();
    }
    ...
    public static void plzGatedBeginUpdate() {
        PLZGatedGrid.beginUpdate();
    }

    public static void plzGatedBeginGroup(String flag, double policy) {
        PLZGatedGrid.beginGroup(flag, (int)Math.floor(policy));
    }

    public static void plzGatedAddRect(double x1, double y1, double x2, double y2, double z) {
        PLZGatedGrid.addRect(x1, y1, x2, y2, z);
    }

    public static int plzGatedEndUpdate() {
        return PLZGatedGrid.endUpdate();
    }

    public static int plzGatedGroupCount() {
        return PLZGatedGrid.getGroupCount();
    }

    public static void plzGatedSetPlayerFlags(String username, String flags) {
        PLZGatedGrid.setPlayerFlags(username, flags);
    }

    public static boolean plzGatedHasFlag(String username, String flag) {
        return PLZGatedGrid.hasFlag(username, flag);
    }

    public static void plzGatedClearPlayerFlags() {
        PLZGatedGrid.clearAllPlayerFlags();
    }

    public static int plzGatedFlaggedPlayerCount() {
        return PLZGatedGrid.getFlaggedPlayerCount();
    }

    public static void plzDoorAccessSet(String username, String keyIds) {
        PLZDoorAccess.set(username, keyIds);
    }

    public static boolean plzDoorAccessHas(String username, double keyId) {
        return PLZDoorAccess.has(username, (int)Math.floor(keyId));
    }

    public static void plzDoorAccessClear() {
        PLZDoorAccess.clearAll();
    }

    public static int plzDoorAccessPlayerCount() {
        return PLZDoorAccess.getGrantedPlayerCount();
    }
    ...
    public static int plzBarrierRefresh() {
        ...
        long[] chunks = PLZBarrierGrid.getChangedChunks();
        int touched = 0;

        for (long key : chunks) {
            int cx = PLZBarrierGrid.chunkX(key);
            int cy = PLZBarrierGrid.chunkY(key);
            IsoChunk chunk = GameServer.server
                ? (ServerMap.instance == null ? null : ServerMap.instance.getChunk(cx, cy))
                : cell.getChunk(cx, cy);
            if (chunk == null) {
                continue;
            }
            ...
            for (int z = chunk.getMinLevel(); z <= chunk.getMaxLevel(); z++) {
                for (int y = 0; y < 8; y++) {
                    for (int x = 0; x < 8; x++) {
                        IsoGridSquare sq = cell.getGridSquare((cx << 3) + x, (cy << 3) + y, z);
                        if (sq != null) {
                            sq.RecalcProperties();
                            sq.RecalcAllWithNeighbours(true);
                            touched++;
        ...
        if (collideObject instanceof IsoPlayer isoPlayer && isoPlayer.isNoClip()) {
            ...
            if (x >= -1 && x <= 1 && y >= -1 && y <= 1 && z >= -1 && z <= 1) {
                ...
                if ((GameServer.server || GameClient.client) && collideObject instanceof IsoPlayer isoPlayer && !(collideObject instanceof IsoAnimal)) {
                    ...
                    if ((GameServer.server || isoPlayer.isLocalPlayer())
                        && PLZGatedGrid.blocksPlayer(this.x + x, this.y + y, this.z + z, isoPlayer)) {
                        return true;
                    }
                }
