...
public class ParticleRectangle {
    ...
    public void render() {
        ...
        if (PLZRainTiles.enabled() && !IsoWeatherFX.debugBounds) {
            WeatherParticleDrawer drawer = IsoWeatherFX.instance.getDrawer(this.id);
            PLZRainTiles.Tile tile = drawer.plzBeginTile();
            ...
            for (int cy = -1; cy < cellsH; cy++) {
                for (int cx = -1; cx < cellsW; cx++) {
                    tile.addOrigin(baseX + cx * this.width, baseY + cy * this.height);
                }
            }
