...
public class WeatherFxMask {
    ...
    public static boolean checkFbos() {
        ...
        int width = plzFxSize(Core.getInstance().getScreenWidth());
        int height = plzFxSize(Core.getInstance().getScreenHeight());
    ...
    public static void addMaskLocation(IsoGridSquare gs, int x, int y, int z) {
        if (!GameServer.server) {
            ...
            if (playerFxMask.requiresUpdate && playerFxMask.hasMaskToDraw && playerFxMask.playerZ == z) {
    ...
    private static void scanForTiles(int nPlayer) {
        ...
        if (!plzScanIsNoop(playerFxMask)) {
            if (!playerFxMask.diamondIterDone) {
                ...
                try (ProfileArea plzArea = profiler.profile("Calc Bounds")) {
                ...
                if (plzScanBuildingOnly(playerFxMask, nPlayer, playerZ)) {
                    return;
                }

                boolean bRender = false;

                try (ProfileArea ox = profiler.profile("scanTriangle")) {
    ...
    private static float plzFxScale() {
        if (IsoPlayer.numPlayers <= 1) {
            int pct = PLZPerf.WEATHER_FX_SCALE_PCT;
            return pct < 100 && pct >= 10 ? pct / 100.0F : 1.0F;
        } else {
            return 1.0F;
        }
    }

    private static int plzFxSize(int screenPixels) {
        float s = plzFxScale();
        return s == 1.0F ? screenPixels : Math.max(1, Math.round(screenPixels * s));
    }

    private static void plzFxViewport() {
        if (plzFxScale() != 1.0F && fboMask != null) {
            SpriteRenderer.instance.glViewport(0, 0, fboMask.getTexture().getWidth(), fboMask.getTexture().getHeight());
        }
    }

    private static boolean plzScanIsNoop(WeatherFxMask.PlayerFxMask playerFxMask) {
        if (!PLZPerf.WEATHER_MASK_IDLE_SKIP) {
            return false;
        } else {
            return playerFxMask.requiresUpdate && playerFxMask.hasMaskToDraw
                ? playerFxMask.player != null && playerFxMask.player.getBuilding() == null && playerFxMask.curIsoWorldRegion == null
                : true;
        }
    }

    private static boolean plzScanBuildingOnly(WeatherFxMask.PlayerFxMask playerFxMask, int nPlayer, int playerZ) {
        if (PLZPerf.WEATHER_MASK_IDLE_SKIP) {
            ...
            if (player != null && playerFxMask.curIsoWorldRegion == null && player.getBuilding() != null && player.getBuilding().getDef() != null) {
                BuildingDef def = player.getBuilding().getDef();
                IsoChunkMap chunkMap = IsoWorld.instance.getCell().getChunkMap(nPlayer);
                int x1 = def.getX() - 1;
                int y1 = def.getY() - 1;
                int x2 = def.getX2() + 1;
    ...
    private static boolean plzMaskIdle(int nPlayer) {
        if (!PLZPerf.WEATHER_MASK_IDLE_SKIP) {
            return false;
        }
        ...
        if (!debugMask && !debugMaskAndParticles) {
            if (!RenderSettings.getInstance().getPlayerSettings(nPlayer).isExterior()) {
                ...
                if (fx.hasCloudsToRender()) {
                    ...
                    return fx.hasFogToRender() && PerformanceSettings.fogQuality == 2
                        ? false
                        : Core.getInstance().getOptionRenderPrecipitation() != 1 || !fx.hasPrecipitationToRender();
                }
            }
        } else {
            return false;
    ...
    public static void renderFxMask(int nPlayer) {
        plzRenderFxMask(nPlayer);
    }

    private static void plzRenderFxMask(int nPlayer) {
        if (!(IsoCamera.frameState.camCharacterZ < 0.0F)
            && DebugOptions.instance.weather.fx.getValue()
            && !GameServer.server
            && IsoWeatherFX.instance != null
            && (LuaManager.thread == null || !LuaManager.thread.step)
            && playerMasks[nPlayer].maskEnabled) {
            WeatherFxMask.PlayerFxMask playerFxMask = playerMasks[curPlayerIndex];
            if (playerFxMask.maskEnabled) {
                ...
                if (maskingEnabled && playerFxMask.hasMaskToDraw && plzMaskIdle(nPlayer)) {
                    return;
                }
                ...
                if (maskingEnabled && playerFxMask.hasMaskToDraw) {
                    ...
                    try (ProfileArea var3x = profiler.profile("scanForTiles")) {
                        scanForTiles(nPlayer);
                    }

                    SpriteRenderer.instance.glIgnoreStyles(true);
                    if (maskingEnabled) {
                        try (ProfileArea var12 = profiler.profile("drawFxMask")) {
                            drawFxMask(nPlayer);
                        }
                    }
                    ...
                    try (ProfileArea var13 = profiler.profile("drawFxLayered")) {
                        if (!RenderSettings.getInstance().getPlayerSettings(nPlayer).isExterior()) {
    ...
    private static void drawFxLayered(int nPlayer, boolean doClouds, boolean doFog, boolean doPrecip) {
        ...
        if (maskingEnabled) {
            ...
            ((Texture)fboMask.getTexture())
                .rendershader2(0.0F, 0.0F, ow, oh, plzFxSize(sx), plzFxSize(sy), plzFxSize(sw), plzFxSize(sh), 1.0F, 1.0F, 1.0F, 1.0F);
            SpriteRenderer.instance.glBlendEquation(32774);
        }
        ...
        float b = 1.0F;
        float a = 1.0F;
        float plzS = plzFxScale();
        float sx1 = sx * plzS / tex.getWidthHW();
        float ey1 = sy * plzS / tex.getHeightHW();
        float ex1 = (sx + sw) * plzS / tex.getWidthHW();
        float sy1 = (sy + sh) * plzS / tex.getHeightHW();
