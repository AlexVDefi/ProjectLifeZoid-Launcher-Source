...
public class UdpConnection extends PacketsCache implements IConnection {
    ...
    public boolean isRelevantTo(float x, float y) {
        if (zombie.plz.PLZFixes.on(zombie.plz.PLZFixes.HANDSHAKE_RELEVANCE) && !this.isFullyConnected()) {
            ...
            // walk does not filter on isFullyConnected(), so the ServerCells whose corners touch
            // world origin claim relevance during EVERY handshake, stay in loadedCells, get
            // reported to the native pop manager, and virtual zombies materialise into the empty
            // origin terrain. Handshaking connections are relevant to nothing.
            //
            // NOTE: this only stops further accumulation. Zombies already materialised at origin
            // have to be swept server-side.
            zombie.plz.PLZFixes.hit(zombie.plz.PLZFixes.HANDSHAKE_RELEVANCE);
            return false;
        }
