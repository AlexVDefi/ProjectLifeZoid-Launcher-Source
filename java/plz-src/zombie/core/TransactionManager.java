...
public class TransactionManager {
    ...
    public static byte isConsistent(
        ...
        if (destination != null
            ...
            if (zombie.plz.PLZFixes.on(zombie.plz.PLZFixes.TRANSACTION_STALE_SWEEP)) {
                plzSweepStaleTransactions();
            }
    ...
    /**
     * PLZ: remove only the cancelling player's transaction.
     *
     * Transaction.lastId is a single byte that wraps every 256 transactions, and
     * removeItemTransaction filters with transactions.removeIf(r -> r.id == id) - byte id alone,
     * no owner check. On a populated server different players routinely hold the same byte id at
     * the same time, so one player cancelling a transfer silently kills every other player's
     * in-flight transfer carrying that id. The item is already gone from the source container by
     * then, so the loser sees it vanish.
     *
     * The owner is resolved from the sending connection rather than from the packet. The client
     * builds its reject with only id and state (TransactionManager.removeItemTransaction, client
     * branch), so the packet's own playerId is still at its default and trusting it would match
     * nothing. Going via the connection is also the safer of the two: a client cannot name a
     * player it does not own, so this cannot be used to cancel someone else's transfer.
     */
    /**
     * PLZ: drop transactions whose containers have gone, before the consistency check reads them.
     *
     * The loop below includes "entry.sourceId.getContainer() == null" as a standalone || branch.
     * Once any in-flight entry's container is unloaded - a player disconnects, a chunk unloads -
     * that branch is true for EVERY subsequent consistency check, so every new inventory-bound
     * transfer from EVERY player is rejected until the stale transaction's endTime lapses. Players
     * see item moves silently refused server-wide for no visible reason.
     *
     * Sweeping is preferred over relaxing the predicate: a transaction whose source or destination
     * container no longer exists can never complete, so removing it is the correct outcome and the
     * check keeps its original meaning for live transactions.
     */
    private static void plzSweepStaleTransactions() {
        boolean removed = transactions.removeIf(transaction -> {
            for (Transaction.TransactionEntry entry : transaction.entries) {
                if (entry.sourceId.getContainer() == null || entry.destinationId.getContainer() == null) {
                    return true;
                }
            }
        ...
        if (removed) {
            zombie.plz.PLZFixes.hit(zombie.plz.PLZFixes.TRANSACTION_STALE_SWEEP);
        }
    }

    public static void plzRemoveRejected(byte id, UdpConnection connection) {
        if (connection == null) {
            return;
        }
        ...
        for (Transaction transaction : transactions) {
            ...
            short owner = transaction.playerId.getID();
            for (IsoPlayer player : connection.players) {
                if (player != null && player.getOnlineID() == owner) {
        ...
        if (owned.isEmpty()) {
            DebugType.Action.debugln("TransactionManager: reject id %d from connection has no matching transaction", id);
            return;
        }

        transactions.removeAll(owned);
        zombie.plz.PLZFixes.hit(zombie.plz.PLZFixes.TRANSACTION_CANCEL);
    }
