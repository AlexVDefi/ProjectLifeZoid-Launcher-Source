...
public class BaseGrappleable implements IGrappleable {
    ...
    private static boolean isForcedGrappleType(String grappleType) {
        return "PLZEscort".equals(grappleType);
    }
    ...
    public void Grappled(IGrappleable grappler, HandWeapon weapon, float grappleEffectiveness, String grappleType) {
        ...
        } else if (!this.canBeGrappled() && !isForcedGrappleType(grappleType)) {
    ...
    public GrappleOffsetBehaviour getGrappleOffsetBehaviour() {
        if (this.isBeingGrappled()) {
            IGrappleable grappler = this.getGrappledBy();
            GrappleOffsetBehaviour inherited = grappler == null ? null : grappler.getGrappleOffsetBehaviour();
    ...
    public void setDoContinueGrapple(boolean doContinueGrapple) {
        if (!doContinueGrapple && this.isGrappling && "PLZEscort".equals(this.sharedGrappleType)) {
            return;
        }
