...
public final class AnimLayer extends PooledObject implements IAnimListener {
    ...
    private void calculateOffsetsForGrappleable(
        ...
        // PLZ. The holder's right, taken before yaw turns the vector, so side never turns the held character.
        float plzRightX = -relativeToForward.y;
        float plzRightY = relativeToForward.x;
        float plzSide = PLZGrappleEdit.sideFor(relativeTo);
        relativeToForward.rotate((float) (Math.PI / 180.0) * offsetRotYaw);
        relativeTo.getPosition(relativeToPos);
        offsetPos.x = relativeToPos.x + relativeToForward.x * offsetForwardDistance + plzRightX * plzSide;
        offsetPos.y = relativeToPos.y + relativeToForward.y * offsetForwardDistance + plzRightY * plzSide;
