...
public final class BuildingRoomsEditor {
    ...
    public void checkBuildingAndRoomIDs(IsoMetaCell metaCell) {
        IdentityHashMap<RoomDef, Integer> plzRoomIndex = null;
        if (PLZPerf.LOADER_CPU_FIXES) {
            plzRoomIndex = new IdentityHashMap<>(metaCell.roomList.size() * 2);

            for (int r = metaCell.roomList.size() - 1; r >= 0; r--) {
                plzRoomIndex.put(metaCell.roomList.get(r), r);
            }
        }
        ...
        for (int i = 0; i < metaCell.buildings.size(); i++) {
            ...
            for (RoomDef roomDef : buildingDef.getRooms()) {
                int index = plzRoomIndex != null ? plzRoomIndex.getOrDefault(roomDef, -1) : metaCell.roomList.indexOf(roomDef);
                if (index == -1) {
    ...
    public void renderMain() {
        if (this.luaEditor != null && this.luaEditor.rawget("javaObject") instanceof UIElement ui && ui.isVisible() && !this.buildings.isEmpty()) {
    ...
    public static void setExposed(Exposer exposer) {
