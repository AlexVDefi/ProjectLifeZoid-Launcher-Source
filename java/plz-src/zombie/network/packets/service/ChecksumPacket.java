...
import zombie.plz.PLZChecksumExempt;
import zombie.scripting.ScriptManager;

/**
 * PLZ shadow of the vanilla class. Two changes, both in the total-checksum exchange:
 *
 * <ul>
 *   <li>the client sends the exemption-filtered total instead of the full one, but only
 *       when the PLZ launcher started it, so the same install joining any other server
 *       behaves exactly like vanilla;
 *   <li>the server accepts EITHER of its two totals, so a client without this patch is
 *       judged on the unfiltered total exactly as it is today.
 * </ul>
 *
 * <p>The wire format is unchanged, so a patched server and a vanilla client still talk.
 * Everything below the total exchange is vanilla.
 */
...
public class ChecksumPacket implements INetworkPacket {
    ...
    public void write(ByteBufferWriter b) {
        if (GameServer.server) {
            ...
            b.putShort(this.pkt);
            if (this.pkt == 1) {
                b.putUTF(PLZChecksumExempt.forSend(GameClient.checksum));
                b.putUTF(PLZChecksumExempt.forSend(ScriptManager.instance.getChecksum()));
                b.putUTF(PLZChecksumExempt.forSend(AdvancedAnimator.getChecksum()));
            }
    ...
    public void parseServer(ByteBufferReader b, UdpConnection connection) {
        ...
        switch (this.pkt) {
            case 1:
                ...
                String serverScript = ScriptManager.instance.getChecksum();
                String serverAnim = AdvancedAnimator.getChecksum();
                this.okLua = PLZChecksumExempt.accepts(luaChecksum, GameServer.checksum);
                this.okScript = PLZChecksumExempt.accepts(scriptChecksum, serverScript);
                this.okAnim = PLZChecksumExempt.accepts(animChecksum, serverAnim);
                System.out.println(
                    "PLZ checksum: user "
                        + connection.getUserName()
                        + " lua="
                        + PLZChecksumExempt.variantOf(luaChecksum, GameServer.checksum)
                        + " script="
                        + PLZChecksumExempt.variantOf(scriptChecksum, serverScript)
                        + " anim="
                        + PLZChecksumExempt.variantOf(animChecksum, serverAnim)
                );
            ...
            case 2:
                ...
                if (firstIndex >= 0 && lastIndex >= firstIndex && lastIndex < firstIndex + 10) {
                    ...
                    while (index <= lastIndex) {
                        ...
                        ByteBufferWriter mismatchBbw = connection.startPacket();
                        PacketTypes.PacketType.Checksum.doPacket(mismatchBbw);
                        this.write(mismatchBbw);
                        PacketTypes.PacketType.Checksum.send(connection);
                        return;
                    }

                    this.setPacketGroupChecksum(firstIndex, true);
                    ByteBufferWriter matchBbw = connection.startPacket();
                    PacketTypes.PacketType.Checksum.doPacket(matchBbw);
                    this.write(matchBbw);
                    PacketTypes.PacketType.Checksum.send(connection);
                } else {
