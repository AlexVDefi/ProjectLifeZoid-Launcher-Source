...
public class PlayerHitObjectPacket extends PlayerHit implements AntiCheatHitLongDistance.IAntiCheat, AntiCheatHitWeapon.IAntiCheat {
    ...
    public void log(UdpConnection connection) {
        if (this.thumpable.getIsoObject().getObjectIndex() == -1) {
            ...
            // PLZ: same line, kept somewhere it will still be there next month.
            // map.txt is a ZLogger file and is emptied at 10MB, and it mixes
            // this in with every other map write. See zombie.plz.PLZConductWatch.
            PLZConductWatch.onObjectDestroyed(
                connection, this.thumpable.getIsoObject(), this.thumpable.getName(), this.weapon.getWeapon().getName()
            );
        }
    }
}
