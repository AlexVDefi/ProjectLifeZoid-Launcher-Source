...
public class ItemTransactionPacket extends Transaction implements INetworkPacket {
    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        if (this.state == Transaction.TransactionState.Request) {
            ...
            if (zombie.plz.PLZFixes.on(zombie.plz.PLZFixes.TRANSACTION_CANCEL)) {
                // PLZ: removeItemTransaction matches on byte id alone, so one player's cancel
                // also drops every other player's in-flight transfer sharing that wrapped id.
                // See TransactionManager.plzRemoveRejected.
                TransactionManager.plzRemoveRejected(this.id, connection);
                return;
            }
