...
public class VehiclePhysicsPacket extends VehicleInterpolationData implements INetworkPacket, AntiCheatSpeed.IAntiCheat {
    ...
    /**
     * PLZ: vanilla leaves {@code sync} as the empty default from INetworkPacket, so when the server
     * cannot resolve this packet's vehicle id the dispatcher logs, calls this, and returns - and the
     * client is told nothing at all. The client's own recovery
     * ({@code processClient} -> {@code VehicleManager.sendVehicleRequest}) only fires when it
     * RECEIVES an inconsistent packet, and a driver is sending, not receiving, so a car the server
     * has lost stays lost. Live saw one vehicle in that state for 37 s continuously.
     *
     * <p>This reports rather than corrects. See {@link zombie.plz.PLZVehicleGhost} for why
     * answering with VehicleRemove would make the common case worse, not better.
     */
    @Override
