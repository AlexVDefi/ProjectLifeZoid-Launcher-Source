...
/**
 * PLZ shadow of the vanilla class. The only change is a SECOND digest.
 *
 * <p>Every file still feeds {@code md}, so the full total is byte-for-byte what vanilla
 * produces and a client without this patch still matches it. Files belonging to an
 * exempt mod are additionally kept out of {@code mdExempt} and out of the GroupOfFiles
 * drill-down, giving a parallel total that both sides can agree on while those mods
 * differ. ChecksumPacket publishes both and accepts either.
 *
 * <p>Note the drill-down that names a file on the kick screen follows the FILTERED set,
 * so for a client without the patch the named file can be the wrong one. That path only
 * runs for a client that is being rejected anyway.
 */
public final class NetChecksum {
    ...
    public static final class Checksummer {
        ...
        private final MessageDigest mdExempt = NetChecksum.makeDigest();

        public void reset() {
            this.md.reset();
            this.mdExempt.reset();
            PLZChecksumExempt.beginBuild();
        }
        ...
        public void addFile(String relPath, String absPath) {
            try {
                ...
                if (PLZChecksumExempt.isExempt(absPath)) {
                    return;
                }

                this.mdExempt.update(bytes, 0, count);
        ...
        public String checksumToString() {
            String full = NetChecksum.HEX.formatHex(this.md.digest());
            PLZChecksumExempt.record(full, NetChecksum.HEX.formatHex(this.mdExempt.digest()));
            return full;
        }
