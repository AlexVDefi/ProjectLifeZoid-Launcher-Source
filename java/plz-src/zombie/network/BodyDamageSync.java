...
public class BodyDamageSync {
    ...
    public void startSendingUpdates(short localId, short remoteID) {
        if (GameServer.server) {
            ...
            if (zombie.plz.PLZFixes.on(zombie.plz.PLZFixes.BODY_DAMAGE_SYNC)) {
                // PLZ: two vanilla defects on this path.
                //
                // First, the lookup above is dereferenced unguarded below (player.getBodyDamage()),
                // so a patient who disconnects between the client sending START_UPDATING and the
                // server processing it becomes an NPE on the packet-processing thread.
                //
                // Second, an unresolvable RECIPIENT registers an Updater whose outbound connection
                // lookup always returns null: it ticks every 500ms forever, sends nothing, and is
                // only cleared by a matching STOP_UPDATING that can no longer arrive. That is the
                // leak the -1 online id produces during the respawn/reconnect race - see the
                // bodyDamageUpdatePacket fix, which repairs the id at parse time; this is the
                // belt-and-braces half for ids that still do not resolve.
                if (player == null || GameServer.IDToPlayerMap.get(remoteID) == null) {
                    zombie.plz.PLZFixes.hit(zombie.plz.PLZFixes.BODY_DAMAGE_SYNC);
                    zombie.debug.DebugLog.log("PLZFixes: refusing BodyDamageSync updater local=" + localId
                        + " remote=" + remoteID + " (unresolved player)");
                    return;
                }
            }
