...
public class ServerMap {
    ...
    // PLZ: releventNow is an ArrayList and its contains() was the membership test on a path a
    // streaming client hits thousands of times a tick. Stamped instead, resolved once per pass in
    // preupdate so the flag can never change under a pass that is already half marked.
    private volatile long plzRelevantPass = 1L;
    private volatile boolean plzRelevantStamped;
    int width;
    int height;
    IsoMetaGrid grid;
    ...
    public void loadOrKeepRelevent(int x, int y) {
        if (!this.isInvalidCell(x, y)) {
            ...
            if (cell == null) {
                ...
                cell.plzRelevantPass = this.plzRelevantPass;
                this.releventNow.add(cell);
            } else if (this.plzRelevantStamped ? cell.plzRelevantPass != this.plzRelevantPass : !this.releventNow.contains(cell)) {
                cell.plzRelevantPass = this.plzRelevantPass;
                this.releventNow.add(cell);
            }
        }
    ...
    public void preupdate() {
        ...
        this.plzRelevantPass++;
        this.plzRelevantStamped = zombie.plz.PLZFixes.on(zombie.plz.PLZFixes.SERVER_MAP_RELEVANCE_STAMP);
        if (this.plzRelevantStamped) {
            zombie.plz.PLZFixes.hit(zombie.plz.PLZFixes.SERVER_MAP_RELEVANCE_STAMP);
        }
    ...
    public void postupdate() {
        ...
        try {
            for (int n = 0; n < this.loadedCells.size(); n++) {
                ServerMap.ServerCell cell = this.loadedCells.get(n);
                boolean relevant = this.plzRelevantStamped ? cell.plzRelevantPass == this.plzRelevantPass : this.releventNow.contains(cell);
                boolean shouldBeLoaded = relevant || !this.outsidePlayerInfluence(cell);
                if (!cell.isLoaded) {
    ...
    public static final class ServerCell {
        // PLZ: the pass this cell was last marked relevant in, replacing releventNow.contains().
        long plzRelevantPass;
        public int wx;
        public int wy;
