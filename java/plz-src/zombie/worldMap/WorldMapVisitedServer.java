...
public class WorldMapVisitedServer {
    ...
    /** BIT_KNOWN (2) in each of the four 2-bit units packed per byte. */
    private static final byte PLZ_ALL_KNOWN = (byte)170;

    /**
     * PLZ: restore the Map.MapAllKnown sandbox option in multiplayer, broken by 42.20.3.
     *
     * 42.20.3 moved a joining player's visited map data from PlayerVisitedPacket to
     * RequestDataPacket.RequestID.PlayerVisited. The old packet's processClient re-applied
     * setKnownInCells over the whole map after overwriting the client's visited array; the new
     * WorldMapVisited.receiveRequestData path dropped that step, so the server's per-user bytes
     * wipe the client's all-known pass and the map shows only where that player has actually been.
     *
     * Setting BIT_KNOWN on the COPY being sent fixes vanilla clients too, since it happens
     * server-side, and the stored per-user data is never mutated - the visited bits are preserved
     * by the OR, so turning the option back off restores real exploration state.
     */
    private static byte[] plzApplyAllKnown(byte[] stored) {
        if (stored == null || stored.length == 0
            || !zombie.plz.PLZFixes.on(zombie.plz.PLZFixes.WORLD_MAP_ALL_KNOWN)) {
            return stored;
        }

        try {
            if (!zombie.SandboxOptions.getInstance().map.mapAllKnown.getValue()) {
                return stored;
            }
        ...
        for (int i = 0; i < stored.length; i++) {
            entity[i] = (byte)(stored[i] | PLZ_ALL_KNOWN);
        }

        zombie.plz.PLZFixes.hit(zombie.plz.PLZFixes.WORLD_MAP_ALL_KNOWN);
        return entity;
    }
    ...
    public void sendRequestData(IConnection connection, ByteBufferWriter b) {
        ...
        entity = plzApplyAllKnown(entity);
        if (entity == null) {
            b.putInt(0);
            b.putInt(0);
        } else {
            ...
            try {
                ...
                while (!deflater.finished() && compressedLength < compressed.length) {
                    int count = deflater.deflate(compressed, compressedLength, compressed.length - compressedLength);
                    if (count == 0) {
                        break;
                    }
