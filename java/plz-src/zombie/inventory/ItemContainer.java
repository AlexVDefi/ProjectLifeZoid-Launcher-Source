...
public final class ItemContainer {
    ...
    // PLZ: bounded, for the reason on getCharacter. Same tests in the same order, one level at a
    // time, so a well-formed chain answers identically.
    public boolean isInCharacterInventory(IsoGameCharacter chr) {
        ...
        for (int depth = 0; depth < MAX_PARENT_DEPTH; depth++) {
            ...
            if (chr.getInventory().contains(container.containingItem, true)) {
                return true;
            }

            if (container.containingItem.getContainer() == null) {
                return false;
            }

            container = container.containingItem.getContainer();
        }

        plzReportParentChainCycle();
        return false;
    }

    // PLZ: bounded, for the reason on getCharacter.
    public boolean isInside(InventoryItem item) {
        ...
        for (int depth = 0; depth < MAX_PARENT_DEPTH; depth++) {
            ...
            if (container.containingItem == item) {
                return true;
            }
    ...
    public InventoryItem AddItem(InventoryItem item) {
        ...
        //
        // Vanilla re-parents here with no ancestry test whatsoever - Remove from the old container,
        // assign the new one - and the only such check in the base game lives in
        // ISInventoryTransferAction, which server-side Lua and any mod calling AddItem directly do
        // not go through. Every route that can build a cycle goes through this one method, so this
        // is the single place that can stop them all being built, whichever mod asks.
        //
        // Traced 2026-08-30 to InventoryTetris, whose drop path validates the dragged item against
        // the grid under the cursor and then transfers it into a DIFFERENT container - the hovered
        // bag's own inventory - and whose one real ancestry check sits behind an `isClient()` early
        // return, so it never runs in multiplayer. That is a client mod bug and this does not depend
        // on it being fixed.
        //
        // Checked BEFORE any mutation, so a refused add leaves the item exactly where it was. The
        // item is returned rather than null because vanilla callers assign this result and go on to
        // use it; handing them null would turn a refused move into an NPE, which is a worse outcome
        // than a move that quietly does not happen. The move failing IS the correct outcome - it is
        // what the vanilla UI guard would have done with the same drop.
        if (this.isInside(item)) {
            plzReportRefusedNesting(this, item);
            return item;
        }
    ...
    // PLZ: walks the parent chain with a bound instead of recursing.
    //
    // The vanilla body is self-recursive - containingItem.getContainer().getCharacter() - with
    // nothing that notices a container which is transitively inside its own inventory. A bag
    // holding itself makes this run until the stack dies, and it dies on whichever thread asked.
    // On a dedicated server that thread is the main loop, so the whole simulation stops while
    // RakNet carries on accepting: the server stays listed and joinable, the frame counter never
    // advances again, no chunks load and nobody can see anybody. Killed live 2026-08-30 with 121
    // players on, as 1024 frames of ItemContainer.getCharacter in DebugLog-server.
    //
    // Nothing in this mod builds such a cycle - every PLZ transfer either creates a fresh item or
    // targets a player's top-level inventory - but the engine offers no protection either:
    // ItemContainer.AddItem re-parents with no ancestry test, and the only guard in the base game
    // is in ISInventoryTransferAction, which server-side Lua does not go through. So any mod on
    // the server can produce it, and a corrupt container already saved to a character reproduces
    // it on every load. Surviving it is worth more than finding it.
    //
    // Same answer as the recursion for every well-formed chain, since it returns at exactly the
    // same two conditions. The bound only decides what a MALFORMED chain returns, and null - "this
    // container is not held by a character" - is what the vanilla code already returns for a chain
    // that runs out without reaching one. Callers all null-check it.
    //
    // MAX_PARENT_DEPTH is far above anything reachable by playing: nesting is bags inside bags, and
    // the containers a player can actually open bottom out in single digits.
    public IsoGameCharacter getCharacter() {
        ItemContainer container = this;

        for (int depth = 0; depth < MAX_PARENT_DEPTH; depth++) {
            if (container.getParent() instanceof IsoGameCharacter) {
                return (IsoGameCharacter)container.getParent();
            }

            if (container.containingItem == null || container.containingItem.getContainer() == null) {
                return null;
            }
    ...
    // PLZ: names the broken chain, so the bound above is a diagnosis and not just a survival.
    //
    // This is why there is no separate scan. A sweep over online players only sees who is logged
    // in, has to be scheduled, and would have to walk a container graph that is by definition
    // malformed. This fires from the exact call that hit the cycle, on any thread, for online and
    // loading characters alike, and costs nothing until something is actually wrong - a well-formed
    // chain returns from the loop and never reaches here.
    //
    // Throttled because a cycle is re-entered from every inventory tick, not once: the first eight
    // are logged in full and then one in every 1024, which is enough to identify the items and
    // never enough to fill a disk. The counter is deliberately not atomic - a lost increment costs
    // one log line and an AtomicLong on a method this hot is not worth paying for a diagnostic.
    private static long plzParentChainReports = 0L;
    private static long plzRefusedNestingReports = 0L;

    // A refusal is a caught bug, not a crash, so it is logged at a lower volume than the cycle
    // report: enough to know which mod and which item, not enough to matter if a UI retries the
    // same rejected drop every frame.
    private static void plzReportRefusedNesting(ItemContainer destination, InventoryItem item) {
        long seen = ++plzRefusedNestingReports;
        if (seen > 4L && seen % 4096L != 0L) {
            return;
        }

        InventoryItem holder = destination.getContainingItem();
        DebugLog.log(
            DebugType.General,
            "PLZ: refused adding " + item.getFullType() + "#" + item.getID()
                + " into a container inside itself (destination held by "
                + (holder == null ? "<no item>" : holder.getFullType() + "#" + holder.getID())
                + ", occurrence " + seen + "). The item was left where it was."
        );
    }

    private void plzReportParentChainCycle() {
        plzReportParentChainCycle(this);
    }

    // Package-private and static so InventoryItem.getOutermostContainer can report through the same
    // throttle. Two classes walk this chain and both can meet the same cycle; one counter between
    // them keeps a single incident to a handful of lines rather than a handful per class.
    static void plzReportParentChainCycle(ItemContainer from) {
        long seen = ++plzParentChainReports;
        if (seen > 8L && seen % 1024L != 0L) {
            return;
        }

        // The ids ARE the fix: they are what an admin removes to break the loop, so the line is
        // worth nothing without them.
        StringBuilder chain = new StringBuilder();
        ItemContainer container = from;
        ...
        for (int depth = 0; depth < 12; depth++) {
            ...
            chain.append(item.getFullType()).append('#').append(item.getID());

            ItemContainer next = item.getContainer();
            if (next == null) {
                break;
            }
            container = next;
        ...
        DebugLog.log(
            DebugType.General,
            "PLZ: container parent chain exceeded " + MAX_PARENT_DEPTH + " (occurrence " + seen
    ...
    // PLZ: bounded, for the reason on getCharacter.
    //
    // Returns THIS on a malformed chain, not the far end of the walk, and that choice is what keeps
    // getSquare() below safe without touching it: getSquare recurses while
    // getOutermostContainer() != this, so answering "I am my own outermost" ends it. Returning the
    // last container reached would instead hand getSquare a different container that also cannot
    // resolve its own outermost, and the two would call each other forever.
    public ItemContainer getOutermostContainer() {
        ItemContainer container = this;

        for (int depth = 0; depth < MAX_PARENT_DEPTH; depth++) {
            InventoryItem held = container.getContainingItem();
            if (held == null) {
                return container;
            }

            ItemContainer outer = held.getContainer();
            if (outer == null) {
                return container;
            }
