...
public class InventoryItem extends GameEntity {
    ...
    // PLZ: the same walk, with an iteration bound. See ItemContainer.getCharacter for the full
    // story; this is the OTHER end of it and the more dangerous half.
    //
    // A container that is transitively inside its own inventory makes the vanilla loop below spin
    // forever: `outer` is reassigned to itself and every condition stays true. There is no stack
    // growth, so no StackOverflowError, so nothing is logged and nothing dies - the thread simply
    // stops. On a dedicated server that is the main loop, and the server then stays listed and
    // joinable while simulating nothing, with no error anywhere to say why.
    //
    // This is exactly what happened on 2026-08-30 the moment the ItemContainer.getCharacter fix
    // went live: the recursion that used to kill the thread with a diagnosable error was gone, so
    // execution reached here instead and hung on the same two self-referencing bags. Bounding one
    // walk and not the other only changes which symptom you get.
    //
    // Terminating early returns the last container reached, which is what the vanilla loop would
    // return had the chain simply ended there - a container, never null, so callers that deref it
    // are unaffected.
    private static final int PLZ_MAX_PARENT_DEPTH = 64;
    ...
    public ItemContainer getOutermostContainer() {
        if (this.container != null && !"floor".equals(this.container.type)) {
            ...
            for (int depth = 0; depth < PLZ_MAX_PARENT_DEPTH; depth++) {
                if (outer.getContainingItem() == null
            ...
            ItemContainer.plzReportParentChainCycle(outer);
            return outer;
        } else {
            return null;
        }
