...
public final class VehiclePart extends GameEntity implements ChatElementOwner, WaveSignalDevice {
    ...
    public void doInventoryItemStats(InventoryItem newItem, int mechanicSkill) {
        if (newItem != null) {
            if (this.isContainer()) {
                // PLZ: a script-declared container capacity belongs to the CAR, so re-stamp it
                // onto whatever item is being fitted. Vanilla only ever does this once, in
                // VehicleUtils.createPartInventoryItem, as the vehicle is first built:
                //
                //     if part:getContainerCapacity() > 0 then
                //         item:setMaxCapacity(part:getContainerCapacity())
                //     end
                //
                // Nothing re-applies it, and the line below reads the ITEM's number, so any
                // later refit hands the part its item type's generic size instead of the car's.
                // Vanilla cars never notice - their scripts declare no container capacity at
                // all, so the item type is the only source of truth. Mod cars do the opposite:
                // KI5 puts the real boot size on the script part (490 on a large cargo trailer,
                // 105 on a Caravan Nomad, 69 on a Charger) and points every one of them at the
                // same shared damnCraft.Trunk item, whose script says 25. Refitting a trunk
                // door through the mechanics menu therefore shrank the boot by up to 95%.
                //
                // Here rather than in the install action because this is the one funnel every
                // path runs through - install, uninstall-then-reinstall, BaseVehicle.repair,
                // the garage rebuild, and VehiclePartItem.parse on the receiving end of the
                // network - so the two realms cannot end up disagreeing about a boot's size.
                // Vanilla's own trailer proves the rule: TrailerTrunk (100), TrailerAnimalFood
                // (20) and TrailerAnimalEggs (10) all fit Base.TrailerTrunk, MaxCapacity 100,
                // so before this a refitted egg slot held ten times what it should.
                int scriptedCapacity = this.getScriptPart().container.capacity;
                if (scriptedCapacity > 0 && newItem.getMaxCapacity() != scriptedCapacity) {
                    newItem.setMaxCapacity(scriptedCapacity);
                }
