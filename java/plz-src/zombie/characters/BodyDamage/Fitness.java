...
public final class Fitness {
    ...
    public void exerciseRepeat() {
        // PLZ: currentExe is set by setCurrentExercise() from a plain exercises.get(type) with no
        // null check, so an exercise name the map does not hold leaves it null. Every one of the
        // five calls below then dereferences it - incRegularity, reduceEndurance,
        // incFutureStiffness, incStats and updateExeTimer all read currentExe.type or
        // currentExe.stiffnessInc - so guarding any single one of them only moves the NPE to the
        // next. This is the one chokepoint that covers all five.
        //
