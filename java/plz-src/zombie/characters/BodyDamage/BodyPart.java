...
import zombie.plz.PLZMuscleStrain;
...
public final class BodyPart {
    ...
    public float getAdditionalPain(boolean includeStiffness) {
        // PLZ PATCH: read through getStiffness() so the gate reaches the one consumer that
        // bypassed it. This is the line that turns stiffness into PAIN - getPain() adds the
        // result, and the Pain moodle and the speed penalties follow from there. The raw field
        // stays writable by the network path (sync(ByteBufferReader, byte) assigns it directly),
        // so masking the getter alone would not cover this unless it reads through the getter.
        return includeStiffness ? this.additionalPain + this.getStiffness() / 3.5F : this.additionalPain;
        // END PLZ PATCH
    }
    ...
    public float getStiffness() {
        // PLZ PATCH: with muscle strain off nothing may see a value. Masking the getter rather
        // than only the setters is what covers the network sync path, which assigns the field
        // directly and would otherwise hand a server's value to every reader. Everything
        // downstream comes through here: getPain, the decay block above, the health panel's
        // "muscle stiffness" line, and BodyDamage.save, which now writes 0 to the wire.
        if (PLZMuscleStrain.isOff()) {
            return 0.0F;
        }
        // END PLZ PATCH
    ...
    public void setStiffness(float stiffness) {
        // PLZ PATCH: force the field to zero rather than ignore the write. Ignoring would leave
        // whatever a save file or an earlier sync had already put there, so raising the option
        // again later would resurrect stiffness earned while it was off. Driving it to 0 means
        // every attempt to apply strain also cleans up after itself, and callers that legitimately
        // clear - the decay block, the admin health panel - get the result they asked for anyway.
        if (PLZMuscleStrain.isOff()) {
            this.stiffness = 0.0F;
            return;
        }
    ...
    public void addStiffness(float stiffness) {
        // PLZ PATCH: the single funnel every strain source ends in - TimedActionScript's crafting
        // strain, Fitness soreness, the hard-fall limb, and the whole add*MuscleStrain family on
        // IsoGameCharacter. Refused here, none of those needs a patch site of its own.
        if (PLZMuscleStrain.isOff()) {
            this.stiffness = 0.0F;
            return;
        }
