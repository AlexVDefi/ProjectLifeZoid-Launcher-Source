...
public final class TileDepthTextures {
    ...
    // Claiming the name is what replaces vanilla's lock: without it two file threads that
    // reach the same tileset both build it and the loser's texture is leaked.
    private final Set<String> plzClaimed = ConcurrentHashMap.newKeySet();
    ...
    /** Vanilla holds the whole-object lock across the PNG decode, so the depth-map tilesets
     *  load strictly one at a time. Decode unlocked, publish under the lock. */
    void plzLoadTileset(String tilesetName) {
        int numRows;
        synchronized (this) {
            if (this.tilesets.get(tilesetName) != null || !this.plzClaimed.add(tilesetName)) {
                return;
            }

            numRows = this.tilesetRows.getOrDefault(tilesetName, 0);
        }
    ...
    static final class LoadTask extends FileTask {
        ...
        public Object call() throws Exception {
            ...
            if (PLZPerf.PARALLEL_DEPTH_MAPS) {
                this.textures.plzLoadTileset(tilesetName);
                return null;
            }
