...
import zombie.plz.PLZChatRecovery;
...
public class ChatManager {
    ...
    public ChatMessage unpackMessage(ByteBufferReader bb) {
        ...
        if (chat == null && PLZFixes.on(PLZFixes.CHAT_JOIN_RECOVERY)) {
            PLZFixes.hit(PLZFixes.CHAT_JOIN_RECOVERY);
            this.plzRequestRejoin("message for unjoined chat " + id);
            return null;
        }
    ...
    public void sendMessageToChat(String author, ChatType type, String msg) {
        ...
        if (!msg.isEmpty()) {
            ...
            if (chat == null) {
                ...
                if (PLZFixes.on(PLZFixes.CHAT_JOIN_RECOVERY)) {
                    PLZFixes.hit(PLZFixes.CHAT_JOIN_RECOVERY);
                    this.plzRequestRejoin("no " + type + " chat to send to");
                }
    ...
    public void showServerChatMessage(String msg) {
        if ((this.serverChat == null || this.focusTab == null) && PLZFixes.on(PLZFixes.CHAT_JOIN_RECOVERY)) {
            PLZFixes.hit(PLZFixes.CHAT_JOIN_RECOVERY);
            DebugLog.log("PLZ: no server chat to show '" + msg + "' in");
            this.plzRequestRejoin(this.serverChat == null ? "server chat missing" : "no focused tab");
            return;
        }
    ...
    // Joins only unless the tabs are gone too: a full re-init re-fires OnChatWindowInit, and
    // ISChat.initChat tears the tab strip down and takes the visible history with it.
    private void plzRequestRejoin(String reason) {
        if (this.singlePlayerMode || !GameClient.client || GameClient.connection == null || this.player == null) {
            return;
        }

        if (!PLZChatRecovery.clientMayAsk()) {
            return;
        }

        boolean full = this.tabs.isEmpty() || this.focusTab == null;
        DebugLog.log("PLZ: chat join incomplete (" + reason + "), asking the server to re-send" + (full ? " everything" : " the joins"));
        GameClient.instance
            .sendClientCommand(
                this.player, PLZChatRecovery.MODULE, full ? PLZChatRecovery.COMMAND_REINIT : PLZChatRecovery.COMMAND_REJOIN, null
            );
    }

    /**
     * The trigger of last resort, polled rather than driven by chat traffic.
     *
     * <p>Every other call to plzRequestRejoin needs a chat packet to arrive or the player to try to
     * send one. A client whose tab strip is gone can do neither - it has no tab to send from, and an
     * inbound message for a chat it still holds never takes the recovery branch - so the one client
     * that most needs re-joining was the one that could never ask.
     *
     * <p>Deliberately fires only once the tabs are already gone. That is the state a re-init would
     * otherwise be destructive in, because ISChat.initChat tears the tab strip down and takes the
     * visible history with it; here there is no history left to lose.
     */
    public void plzHealthCheck() {
        if (this.singlePlayerMode || !GameClient.client || GameClient.connection == null || this.player == null) {
            return;
        }
        ...
        if (!this.tabs.isEmpty() && this.focusTab != null) {
            PLZChatRecovery.clientChatOk();
            return;
        }

        // Only after the check above, so the clock measures unbroken breakage rather than uptime.
        if (!PLZChatRecovery.clientSettled()) {
            return;
        }

        PLZFixes.hit(PLZFixes.CHAT_JOIN_RECOVERY);
        this.plzRequestRejoin(this.tabs.isEmpty() ? "health check: no chat tabs" : "health check: no focused tab");
    }
