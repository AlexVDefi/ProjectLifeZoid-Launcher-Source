...
public abstract class ChatBase {
    ...
    /**
     * PLZ: public view of hasMember, so the server-only ChatServer.disconnectPlayer can ask
     * whether the leaver was actually in this chat before deciding to close it.
     *
     * Added rather than widening hasMember itself: the shadow fidelity gate compares javap
     * output including access modifiers, so changing private to public reads as a dropped
     * member. Extra members are fine, altered ones are not.
     */
    public boolean plzHasMember(Short playerID) {
        synchronized (this.memberLock) {
            return this.hasMember(playerID);
        }
    }
