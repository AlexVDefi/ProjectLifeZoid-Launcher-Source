...
public class SpriteConfig extends Component {
    ...
    protected void onAddedToOwner() {
        if (zombie.plz.PLZFixes.on(zombie.plz.PLZFixes.SPRITE_CONFIG_IDEMPOTENT)
            && this.objectInfo != null
            && this.faceInfo != null
            && this.tileInfo != null) {
            // PLZ: GameEntity.receiveSyncEntity calls this on every network sync, and vanilla
            // rebuilds the object/face/tile info unconditionally. When the sprite is in a
            // non-default state - an open door, a smashed window - the rebuild cannot resolve
            // the face and logs "Invalid SpriteConfig object!" every sync. Already-initialised
            // is already correct, so there is nothing to redo.
            zombie.plz.PLZFixes.hit(zombie.plz.PLZFixes.SPRITE_CONFIG_IDEMPOTENT);
            return;
        }
